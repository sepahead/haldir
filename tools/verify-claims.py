...
MUTATION_TEST_PATH = ROOT / "tools" / "test_verify_claims.py"
EXPECTED_MUTATION_TESTS = 20
...
def mutation_suite_problems() -> list[str]:
    """Run the causal verifier controls through the existing CI entry point."""

    spec = importlib.util.spec_from_file_location(
        "verify_claims_mutation_suite", MUTATION_TEST_PATH
    )
    if spec is None or spec.loader is None:
        return [f"cannot load mutation suite {MUTATION_TEST_PATH.relative_to(ROOT)}"]
    module = importlib.util.module_from_spec(spec)
    try:
        spec.loader.exec_module(module)
    except (ImportError, OSError, RuntimeError, SyntaxError) as error:
        return [f"mutation suite import failed: {error}"]

    suite = unittest.defaultTestLoader.loadTestsFromModule(module)
    stream = io.StringIO()
    result = unittest.TextTestRunner(stream=stream, verbosity=1).run(suite)
    problems: list[str] = []
    if result.testsRun != EXPECTED_MUTATION_TESTS:
        problems.append(
            "mutation suite count drift: "
            f"expected {EXPECTED_MUTATION_TESTS}, observed {result.testsRun}"
        )
    if not result.wasSuccessful():
        output = " ".join(stream.getvalue().split())
        problems.append(f"mutation suite failed: {output[:2_000]}")
    return problems
...
def main() -> None:
    problems = 0
    for problem in mutation_suite_problems():
