#!/usr/bin/env python3
"""Verify the append-only FR-0016 epoch-17 trust-root transition.

FR-0015 remains the signed, activated epoch-16 historical boundary.  Its
verifier and every earlier recovery verifier are inert here.  This bridge
freezes and verifies the exact signed FR-0015 R/Q/A boundary and every signed
linear successor through M16 directly, then validates only the deterministic
FR-0016 R/Q/A sequence and ordinary signed linear milestones after activation.

Epoch-17 hosted-result artifacts convey provenance only and grant no release
authority.
"""
...
PARENT = "ec207cbdba36f15a410206fd01e76a54ffd29ac0"
PARENT_TREE = "1b7e4b32576fab770aeaee0770335af50544149c"
PARENT_PARENT = "d441316189acad8fd2063d523399021684a2d0ca"
PARENT_SUBJECT = "plant: make runtime transitions fail closed"
...
RECOVERY_ID = "FR-0016"
REPAIR_SUBJECT = "release: establish epoch-17 audit trust root"
QUALIFICATION_SUBJECT = "release: qualify epoch-17 audit trust root"
ACTIVATION_SUBJECT = "release: activate epoch-17 audit trust root"
PLAN_NAMESPACE = "haldir-framework-recovery-fr-0016-plan-v1"
...
QUALIFICATION_NAMESPACE = "haldir-framework-recovery-fr-0016-qualification-v1"
ACTIVATION_NAMESPACE = "haldir-framework-recovery-fr-0016-activation-v1"
...
PLAN_PATH = "release/0.9.0/current-head/closures/framework-recovery/FR-0016-plan.json"
QUALIFICATION_PATH = (
    "release/0.9.0/current-head/closures/framework-recovery/FR-0016-qualification.json"
)
ACTIVATION_PATH = (
    "release/0.9.0/current-head/closures/framework-recovery/FR-0016-activation.json"
)
FR0015_PLAN_PATH = (
...
MODULE_PATH = "tools/release/framework_recovery_fr_0016.py"
CAPTURE_PATH = "tools/release/framework_recovery_fr_0016_capture.py"
RESULT_PATH = "tools/release/framework_recovery_fr_0016_result.py"
BRIDGE_PATH = "tools/release/verify-framework-recovery-fr-0016.py"
TEST_PATH = "tools/release/test_verify_framework_recovery_fr_0016.py"
...
REPAIR_STATUSES = {
    ...
    FORMAL_RUNNER_PATH: "M",
    FORMAL_RUNNER_TEST_PATH: "M",
    FORMAL_README_PATH: "M",
    JUSTFILE_PATH: "M",
    GATE_PATH: "M",
    MODULE_PATH: "A",
    CAPTURE_PATH: "A",
...
REPAIR_CI_PATHS = (
    f"{EVIDENCE_ROOT}/framework-recovery-fr-0016-r-ci-capture.json",
    f"{EVIDENCE_ROOT}/framework-recovery-fr-0016-r-ci-result.json",
    f"{EVIDENCE_ROOT}/framework-recovery-fr-0016-r-ci-attestation.json",
)
REPAIR_FORMAL_PATHS = (
    f"{EVIDENCE_ROOT}/framework-recovery-fr-0016-r-formal-capture.json",
    f"{EVIDENCE_ROOT}/framework-recovery-fr-0016-r-formal-result.json",
    f"{EVIDENCE_ROOT}/framework-recovery-fr-0016-r-formal-attestation.json",
)
LOCAL_PATH = f"{EVIDENCE_ROOT}/framework-recovery-fr-0016-r-local.json"
PULL_REQUEST_PATH = f"{EVIDENCE_ROOT}/framework-recovery-fr-0016-r-pull-request.json"
...
QUALIFICATION_CI_PATHS = (
    f"{EVIDENCE_ROOT}/framework-recovery-fr-0016-q-ci-capture.json",
    f"{EVIDENCE_ROOT}/framework-recovery-fr-0016-q-ci-result.json",
    f"{EVIDENCE_ROOT}/framework-recovery-fr-0016-q-ci-attestation.json",
)
QUALIFICATION_FORMAL_PATHS = (
    f"{EVIDENCE_ROOT}/framework-recovery-fr-0016-q-formal-capture.json",
    f"{EVIDENCE_ROOT}/framework-recovery-fr-0016-q-formal-result.json",
    f"{EVIDENCE_ROOT}/framework-recovery-fr-0016-q-formal-attestation.json",
)
...
class BridgeError(RuntimeError):
    """One fail-closed epoch-17 bridge error."""
...
def _git(
    ...
    *arguments: str,
    ...
    if (
        ...
        _fail("FR0016_GIT:" + (arguments[0] if arguments else "missing"))
...
def _verify_provisional_commit_identity(
    ...
    *,
    ...
    if (
        ...
        _fail("FR0016_PROVISIONAL_COMMIT_IDENTITY")
    return metadata
...
def _verify_detached(
    ...
    *,
    ...
    if (
        ...
        _fail("FR0016_DETACHED_SIGNATURE")
    try:
        signature_payload = record["signature"].encode("ascii")
    except UnicodeEncodeError:
        _fail("FR0016_DETACHED_SIGNATURE")
    with tempfile.TemporaryDirectory(prefix="haldir-fr0016-signature-") as name:
...
def _validate_authority(
    ...
    *,
    ...
    if (
        ...
        _fail("FR0016_AUTHORITY_SCHEMA")


def expected_plan(repo: Path, repair_commit: str) -> dict[str, Any]:
    """Return the exact unsigned FR-0016 repair plan."""
    ...
    limitations = [
        ...
        (
            "The epoch-17 artifact syntax is an evidence transport only and "
        ...
        (
            "Epoch 17 performs no branch-control or hosted-settings mutation. "
    ...
    return {
        ...
        "legacy_boundary": {
            "state": "HISTORICAL_SIGNED_FR_0015_ACTIVE_BOUNDARY",
            "commit": PARENT,
            "tree": PARENT_TREE,
            ...
            "repair_parent": FR0015_REPAIR_PARENT,
            "repair_subject": FR0015_REPAIR_SUBJECT,
            ...
            "fr_0015_state": "ACTIVE",
            "fr_0015_successor_verifier_state": "RETIRED_AT_FR_0016_REPAIR",
            "fr_0015_successor_verifier_executed": False,
            ...
            "records": copy.deepcopy(FR0015_BOUNDARY_RECORDS),
        },
        "defects": [
            {
                "id": "FR0016-D01",
                "summary": (
                    "The epoch-16 signed-linear current-audit gate rejects "
                    "GitHub's two-parent synthetic pull-request merge commit, "
                    "so the seven protected pre-accept checks cannot all run "
                    "against the merge result."
                ),
                "prior_state": "PULL_REQUEST_MERGE_RESULT_STRUCTURALLY_REJECTED",
                "authority_produced": False,
            },
            {
                "id": "FR0016-D02",
                "summary": (
                    "Workflow concurrency uses cancel-in-progress for main, "
                    "allowing a later run to cancel or coalesce a canonical "
                    "main evidence-producing run."
                ),
                "prior_state": "MAIN_RUN_CANCELLATION_OR_COALESCING_POSSIBLE",
                "required_state": "EVERY_MAIN_RUN_UNIQUE_AND_NONCANCELLABLE",
            },
            {
                "id": "FR0016-D03",
                "summary": (
                    "The formal workflow delegates Java acquisition to resolver "
                    "inputs instead of binding exact Temurin archive bytes and "
                    "runtime properties, omits the TLA+ byte length, uses a "
                    "weaker download envelope, and pipes Java through tee "
                    "without an explicit pipefail shell contract."
                ),
                "prior_state": (
                    "FORMAL_RUNTIME_PROVENANCE_AND_PRODUCER_STATUS_NOT_FULLY_CLOSED"
                ),
                "required_state": (
                    "EXACT_RUNTIME_ARCHIVE_SAFE_EXTRACTION_IDENTITY_AND_PIPELINE"
                ),
            },
            {
                "id": "FR0016-D04",
                "summary": (
                    "The pin source and local formal runner lack one closed "
                    "schema that binds the exact hosted Java archive and "
                    "installed runtime properties, verified TLA+ asset, "
                    "observed local Java runtime identity, and portable local "
                    "architecture vocabulary."
                ),
                "prior_state": "FORMAL_PIN_AND_LOCAL_RUNTIME_CONTRACT_INCOMPLETE",
                "required_state": "SCHEMA_V3_AND_FORMAL_RUNTIME_V2",
            },
        ],
        "correction": {
            "normative_qualification_evidence": [
                "SIGNED_FR_0015_ACTIVE_BOUNDARY_AND_LINEAR_SUCCESSORS",
                "FR_0016_R_TEMPORARY_PR_TWO_PARENT_MERGE_ALL_REQUIRED_CHECKS",
                "FR_0016_R_MAIN_CI_RESULT_AND_GITHUB_OIDC_ATTESTATION",
                "FR_0016_R_MAIN_FORMAL_RESULT_AND_GITHUB_OIDC_ATTESTATION",
                "FR_0016_R_EXACT_LOCAL_VALIDATION",
            ...
            "pull_request_mode": {
                ...
                "trusted_event_only_surfaces_skipped": [
                    ...
                    "EPOCH_17_RESULT_EMIT_AND_UPLOAD",
                    "OIDC_ATTESTATION",
                ],
                ...
                "epoch_17_recovery_primitive_suite_skipped": False,
            ...
            "hosted_result_transport": {
                "protocol": "HALDIR_EPOCH_17_HOSTED_RESULT_V1",
                "state": "EPOCH_17_PROVENANCE_FORMAT",
                "governance_epoch": 17,
                "authority_conferred": False,
            },
            "hosted_settings_capture": {
                ...
                "observed_commit": "SIGNED_FR_0016_QUALIFICATION_COMMIT",
            ...
            "ecosystem_pin_contract": {
                "fr0016_change_reason": (
                    "CLOSE_PR_CONCURRENCY_FORMAL_RUNTIME_AND_HOSTED_POLICY_GAPS"
                ),
                ...
                "cargo_deny_direct_execution": {
                    "state": "INHERITED_FROM_SIGNED_FR_0015_BOUNDARY",
                    "version": "0.20.2",
                    "action_uses": 0,
...
def _verify_legacy_boundary(repo: Path) -> None:
    ...
    if _metadata(repo, FR0015_QUALIFICATION)["tree"] != FR0015_QUALIFICATION_TREE:
        _fail("FR0016_FR0015_QUALIFICATION_TREE")
    _verify_commit_identity(
        repo,
    ...
    if _metadata(repo, FR0015_ACTIVATION)["tree"] != FR0015_ACTIVATION_TREE:
        _fail("FR0016_FR0015_ACTIVATION_TREE")
    _verify_commit_identity(
        repo,
        FR0015_ACTIVATION,
    ...
    for commit, parent, tree, subject in FR0015_SUCCESSORS:
        if _metadata(repo, commit)["tree"] != tree:
            _fail("FR0016_FR0015_SUCCESSOR_TREE:" + commit)
        ...
        if (
            ...
            _fail("FR0016_FR0015_SUCCESSOR_SCOPE:" + commit)
    for path, expected in FR0015_BOUNDARY_RECORDS.items():
        ...
        if comparable != expected:
            _fail("FR0016_LEGACY_RECORD:" + path)
    _assert_absent(
        repo,
        PARENT,
        sorted(
    ...
    if (
        ...
        _fail("FR0016_FR0015_PLAN_STATE")
    _validate_authority(
        plan.get("authority"),
    ...
    if (
        ...
        _fail("FR0016_FR0015_QUALIFICATION_STATE")
    _validate_authority(
    ...
    if (
        ...
        _fail("FR0016_FR0015_ACTIVATION_STATE")
    _validate_authority(
...
def _load_protocol_module(repo: Path, repair_commit: str) -> ModuleType:
    ...
    try:
        ...
        _fail("FR0016_MODULE_WORKTREE_DRIFT")
    if current != payload:
        _fail("FR0016_MODULE_WORKTREE_DRIFT")
    specification = importlib.util.spec_from_file_location(
        "_haldir_fr0016_protocol", repo / MODULE_PATH
    )
    if specification is None or specification.loader is None:
        _fail("FR0016_MODULE_LOAD")
    module = importlib.util.module_from_spec(specification)
    specification.loader.exec_module(module)
    if (
        module.RESULT_PROTOCOL != "HALDIR_EPOCH_17_HOSTED_RESULT_V1"
        or module.MAX_EPOCH17_RUN_ATTEMPT != 8
    ):
        _fail("FR0016_MODULE_CONTRACT")
    return module
...
def _trusted_gh() -> tuple[Path, str]:
    if sys.platform == "darwin":
        ...
        _fail("FR0016_GH_EXECUTABLE")
    configured = os.environ.get("HALDIR_FR0016_GH")
    ...
    if executable is None:
        _fail("FR0016_GH_EXECUTABLE")
    with tempfile.TemporaryDirectory(prefix="haldir-fr0016-gh-version-") as name:
        completed = subprocess.run(
    ...
    try:
        ...
        if stdout_pipe is None or stderr_pipe is None:
            ...
            while selector.get_map() and failure is None:
                ...
                if remaining_time <= 0:
                    failure = (
                        "FR0016_PROCESS_CLEANUP"
                        if leader_exited
                        else "FR0016_PROCESS_TIMEOUT"
                    )
                    break
                ...
                for key, _mask in ready:
                    ...
                    if consumed > output_limit:
                        failure = "FR0016_PROCESS_OUTPUT_BOUND"
                        break
            ...
            while not leader_exited and failure is None:
                ...
                if remaining_time <= 0:
                    failure = "FR0016_PROCESS_TIMEOUT"
                    break
            ...
            if failure is None and leader_exited:
                ...
                try:
                    ...
                    failure = "FR0016_PROCESS_REAP"
    except Exception:
        failure = "FR0016_PROCESS_CLEANUP"
        ...
        try:
            ...
            try:
                ...
                cleanup_ok = False
    if not cleanup_ok:
        _fail("FR0016_PROCESS_CLEANUP")
    if unexpected is not None:
        raise unexpected
    if failure is not None:
        _fail(failure)
    if returncode is None or streams is None:
        _fail("FR0016_PROCESS_REAP")
...
def _validate_trusted_root(payload: bytes) -> None:
    if (
        ...
        _fail("FR0016_TRUSTED_ROOT_BOUND")
    ...
    try:
        ...
        _fail("FR0016_TRUSTED_ROOT_JSONL")
    if (
        not isinstance(value, dict)
        ...
        _fail("FR0016_TRUSTED_ROOT_IDENTITY")
...
def _verify_attestation_offline(
    *,
    ...
    with tempfile.TemporaryDirectory(prefix="haldir-fr0016-attestation-") as name:
        root = Path(name)
        result_path = root / f"epoch-17-{workflow}-result-attempt-{attempt}.json"
    ...
    if returncode != 0 or stderr:
        _fail("FR0016_ATTESTATION_CRYPTOGRAPHY")
    try:
...
def _hosted_commands(
    *,
    ...
    artifact_name = f"epoch-17-{workflow}-result-attempt-{attempt}.json"
    return {
...
def _validate_hosted_lane(
    ...
    *,
    ...
    if (
        ...
        or capture["protocol"] != "HALDIR_FR_0016_HOSTED_RESULT_CAPTURE_V1"
    ...
    if (
        ...
        _fail("FR0016_ARTIFACT_UNIQUENESS")
    if capture["artifact_download"] != {
        ...
        _fail("FR0016_ARTIFACT_DOWNLOAD")
    ...
    if captured < max(
        ...
        _fail("FR0016_HOSTED_CHRONOLOGY")
    materials = [
    ...
    if (
        ...
        _fail("FR0016_CAPTURE_OFFLINE_VERIFICATION")
    return {
        "workflow": workflow,
        "run_id": run["run_id"],
        "attempt": run["attempt"],
...
def _validate_run_pull_request_association(
    ...
    *,
    ...
    if (
        ...
        _fail("FR0016_PULL_REQUEST_RUN_ASSOCIATION")
    return copy.deepcopy(value)
...
def validate_pull_request_evidence(
    ...
    *,
    ...
    if (
        ...
        or value["protocol"] != "HALDIR_FR_0016_PULL_REQUEST_QUALIFICATION_V1"
    ):
        _fail("FR0016_PULL_REQUEST_SCHEMA")
    ...
    if not isinstance(pull_request, dict) or set(pull_request) != pull_fields:
        _fail("FR0016_PULL_REQUEST_RECORD")
    ...
    if (
        ...
        _fail("FR0016_PULL_REQUEST_REFS")
    ...
    if not created <= closed <= updated:
        _fail("FR0016_PULL_REQUEST_CHRONOLOGY")
    ...
    if (
        ...
        _fail("FR0016_PULL_REQUEST_MERGE")
    runs = value["runs"]
    if not isinstance(runs, dict) or set(runs) != {"ci", "formal"}:
        _fail("FR0016_PULL_REQUEST_RUNS")
    validated_runs = {
    ...
    if (
        ...
        _fail("FR0016_PULL_REQUEST_OPEN_OBSERVATION")
    ...
    if not created <= open_updated <= association_captured <= closed:
        _fail("FR0016_PULL_REQUEST_OPEN_CHRONOLOGY")
    for workflow in ("ci", "formal"):
    ...
    for run in validated_runs.values():
        if run["created"] < created or run["updated"] > closed:
            _fail("FR0016_PULL_REQUEST_RUN_CHRONOLOGY")
    ...
    if (
        ...
        _fail("FR0016_PULL_REQUEST_EVENT_CONTRACT")
    ...
    if (
        ...
        _fail("FR0016_PULL_REQUEST_CAPTURE")
    ...
    if captured < closed or captured > _parse_git_time(
        ...
        _fail("FR0016_PULL_REQUEST_CAPTURE_CHRONOLOGY")
    expected_authority = {
        "durable_external_state_proof": False,
    ...
    if (
        ...
        _fail("FR0016_PULL_REQUEST_AUTHORITY")
    return copy.deepcopy(value)
...
def validate_hosted_settings_capture(
    ...
    *,
    ...
    if (
        ...
        or set(value) != fields
        or value["schema_version"] != "1.0.0"
        or value["protocol"] != "HALDIR_FR_0016_HOSTED_SETTINGS_CAPTURE_V1"
        or value["observed_commit"] != qualification_commit
    ):
        _fail("FR0016_HOSTED_SETTINGS_SCHEMA")
    validate_repository_identity(value["repository"])
    ...
    if (
        ...
        _fail("FR0016_HOSTED_SETTINGS_HEAD_STABILITY")
    ...
    if (
        ...
        _fail("FR0016_HOSTED_SETTINGS_POLICY")
    ...
    if (
        ...
        _fail("FR0016_HOSTED_SETTINGS_HISTORY_SCOPE")
    capture = value["capture"]
    if (
        not isinstance(capture, dict)
        or set(capture) != {"captured_at_utc", "commands", "result", "transport"}
        ...
        _fail("FR0016_HOSTED_SETTINGS_CAPTURE")
    ...
    if captured < _parse_git_time(
        ...
        _fail("FR0016_HOSTED_SETTINGS_CHRONOLOGY")
    expected_authority = {
        "cryptographic_proof": False,
        "durable_external_state_proof": False,
        "release_authority": False,
    ...
    if (
        ...
        _fail("FR0016_HOSTED_SETTINGS_AUTHORITY")
    return copy.deepcopy(value)
...
def validate_main_writer_ruleset(
    ...
    if created > updated:
        _fail("FR0016_RULESET_CHRONOLOGY")
    bypass_actors = [
        {
    ...
    if (
        ...
        _fail("FR0016_RULESET_EFFECTIVE")
    if (
...
def validate_branch_protection_get(value: Any) -> dict[str, Any]:
    ...
    if not isinstance(value, dict) or set(value) != set(expected):
        _fail("FR0016_BRANCH_PROTECTION_POLICY")
...
def _verify_branch_protection(
    ...
    if (
        ...
        or value["schema_version"] != "1.0.0"
        or value["protocol"] != "HALDIR_FR_0016_BRANCH_PROTECTION_CAPTURE_V1"
        ...
        _fail("FR0016_BRANCH_PROTECTION_SCHEMA")
    ...
    if (
        ...
        _fail("FR0016_BRANCH_PROTECTION_HEAD_STABILITY")
    ...
    if (
        ...
        _fail("FR0016_BRANCH_PROTECTION_CAPTURE")
    ...
    if captured < _parse_git_time(
        ...
        _fail("FR0016_BRANCH_PROTECTION_CHRONOLOGY")
...
def _verify_local_evidence(
    ...
    if (
        ...
        or local["protocol"] != "HALDIR_FR_0016_LOCAL_VALIDATION_V1"
...
def _verify_qualification(
    ...
    *,
    ...
    if unsigned != expected:
        _fail("FR0016_QUALIFICATION_RECORD")
    return value
...
def _verify_activation(
    ...
    *,
    ...
    if _changed_statuses(repo, qualification_commit, commit) != dict(
        ...
        _fail("FR0016_ACTIVATION_DIFF")
    for path in (ACTIVATION_PATH, *ACTIVATION_EVIDENCE_PATHS):
        if _tree_entry(repo, commit, path)["mode"] != "100644":
            _fail("FR0016_ACTIVATION_MODE:" + path)
    for path in (
        *CORE_PATHS,
        PLAN_PATH,
        QUALIFICATION_PATH,
        ...
        *FR0015_BOUNDARY_RECORDS,
        ...
        if _tree_entry(repo, commit, path) != _tree_entry(
            ...
            _fail("FR0016_ACTIVATION_DRIFT:" + path)
    _assert_absent(
        repo,
        commit,
        sorted(
    ...
    if unsigned != expected:
        _fail("FR0016_ACTIVATION_RECORD")
    return value
...
def verify(repo: Path) -> dict[str, Any]:
    """Verify the current first-parent epoch-17 state."""
...
def expected_record_for_provisional(
    ...
    *,
    ...
    if HEX40.fullmatch(parent) is None:
        _fail("FR0016_PROVISIONAL_PARENT")
    ...
    _fail("FR0016_PROVISIONAL_STAGE")
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        print(f"verify-framework-recovery-fr-0016: {error}", file=sys.stderr)
        return 1
    print(
        "verify-framework-recovery-fr-0016: OK "
        f"({result['state']}; epoch 17; release NO_GO)"
    )
    return 0
