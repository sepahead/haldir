#!/usr/bin/env python3
"""Emit the canonical epoch-14 hosted-check result artifact.
...
def _file_record(repo: Path, commit: str, path: str) -> dict[str, Any]:
    ...
    if not raw.endswith(b"\0") or raw.count(b"\0") != 1:
        _fail("FR0013_RESULT_MATERIAL:" + path)
    try:
...
def build_result(
    ...
    *,
    ...
    if contract is None:
        _fail("FR0013_RESULT_WORKFLOW")
    if _exact_environment(environment, "GITHUB_ACTIONS") != "true":
        _fail("FR0013_RESULT_NOT_ACTIONS")
    ...
    if run_attempt > 8:
        _fail("FR0013_RESULT_RUN_ATTEMPT")
    return {
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        _write_exclusive(
            ...
            expected_name=(
                f"epoch-14-{arguments.workflow}-result-attempt-"
        ...
        print(f"framework-recovery-fr-0013-result: {error}", file=sys.stderr)
        return 1
    print(
        "framework-recovery-fr-0013-result: OK "
