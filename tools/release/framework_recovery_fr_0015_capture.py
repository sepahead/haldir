#!/usr/bin/env python3
"""Capture bounded FR-0015 evidence using exact GitHub identities.
...
PROTOCOL_PATH = "tools/release/framework_recovery_fr_0015.py"
...
def _git(repo: Path, *arguments: str, limit: int = MAX_JSON_BYTES) -> bytes:
    ...
    if completed.returncode != 0 or completed.stderr or len(completed.stdout) > limit:
        _fail("FR0015_CAPTURE_GIT:" + (arguments[0] if arguments else "missing"))
...
def _repository() -> Path:
    ...
    try:
        ...
        _fail("FR0015_CAPTURE_REPOSITORY")
...
def _stage_paths(bridge: ModuleType, stage: str, workflow: str) -> tuple[str, str, str]:
    ...
    _fail("FR0015_CAPTURE_STAGE")


def _run_fields(*, epoch16: bool, attempt: bool) -> str:
    fields = [
        "attempt",
        "conclusion",
        "createdAt",
...
def _capture_hosted(
    ...
    if any(path.exists() or path.is_symlink() for path in output_paths):
        _fail("FR0015_CAPTURE_OUTPUT_EXISTS")
    if arguments.run_id < 1:
        _fail("FR0015_CAPTURE_RUN_ID")
    ...
    ordinary_fields = _run_fields(epoch16=True, attempt=False)
    ...
    if (
        ...
        or not 1 <= attempt_number <= protocol.MAX_EPOCH16_RUN_ATTEMPT
    ):
        _fail("FR0015_CAPTURE_ATTEMPT")
    attempt_fields = _run_fields(epoch16=True, attempt=True)
    attempt = _json_bytes(
        _gh(
            bridge,
            executable,
    ...
    artifact_name = f"epoch-16-{workflow}-result-attempt-{attempt_number}.json"
    listing_endpoint = (
    ...
    if type(artifact_id) is not int or artifact_id < 1:
        _fail("FR0015_CAPTURE_ARTIFACT_ID")
    ...
    if artifact_by_id != artifact:
        _fail("FR0015_CAPTURE_ARTIFACT_IDENTITY")
    result_payload = _gh(
        bridge,
        executable,
        (
    ...
    if not result_payload.endswith(b"\n"):
        _fail("FR0015_CAPTURE_DIRECT_ARTIFACT")
    materials = [
    ...
    with tempfile.TemporaryDirectory(prefix="haldir-fr0015-download-") as name:
        root = Path(name)
        ...
        if len(bundles) != 1:
            _fail("FR0015_CAPTURE_ATTESTATION_FILE")
        bundle_payload = _read_regular_bounded(
            bundles[0],
            limit=MAX_BUNDLE_BYTES,
            label="FR0015_CAPTURE_ATTESTATION_BOUND",
        )
    ...
    capture = {
        ...
        "protocol": "HALDIR_FR_0015_HOSTED_RESULT_CAPTURE_V1",
        "workflow": workflow,
        "subject_commit": subject_commit,
        "subject_tree": subject_tree,
        "expected_ref": "refs/heads/main",
    ...
    print(
        "framework-recovery-fr-0015-capture: OK "
...
def _normalize_repository_document(
    ...
    if (
        ...
        _fail("FR0015_CAPTURE_REPOSITORY_IDENTITY")
    identity = {
        "id": document["id"],
...
def _capture_branch_protection(
    ...
    if observed != arguments.qualification_commit:
        _fail("FR0015_CAPTURE_PROTECTION_HEAD")
    ...
    if (
        ...
        _fail("FR0015_CAPTURE_RULESET_HISTORY")
    ...
    try:
        ...
        _fail("FR0015_CAPTURE_BRANCH_CONTROL_VALIDATION")
    ...
    value = {
        "schema_version": "1.0.0",
        "protocol": "HALDIR_FR_0015_BRANCH_PROTECTION_CAPTURE_V1",
...
def _assemble_signed_record(
    ...
    unsigned = _read_regular_bounded(
        arguments.unsigned,
        limit=MAX_JSON_BYTES,
        label="FR0015_CAPTURE_UNSIGNED_RECORD",
    )
    signature_payload = _read_regular_bounded(
        arguments.signature,
        limit=16 * 1024,
        label="FR0015_CAPTURE_RECORD_SIGNATURE",
    )
    try:
        ...
        _fail("FR0015_CAPTURE_UNSIGNED_RECORD")
...
def _resolve_local_cargo(
    ...
    if candidate is None:
        _fail("FR0015_CAPTURE_LOCAL_RUSTUP")
    rustup, rustup_payload = _validated_local_executable(
        repo,
        Path(candidate),
        label="FR0015_CAPTURE_LOCAL_RUSTUP",
    )
    ...
    if returncode != 0 or stderr or stdout.count(b"\n") != 1:
        _fail("FR0015_CAPTURE_LOCAL_CARGO")
    try:
    ...
    cargo, cargo_payload = _validated_local_executable(
        repo,
        cargo_candidate,
        label="FR0015_CAPTURE_LOCAL_CARGO",
    )
    cargo_environment = {
        "LC_ALL": "C",
...
def _capture_local(
    ...
    if (
        ...
        _fail("FR0015_CAPTURE_LOCAL_HEAD")
    ...
    if "HALDIR_FR0015_GH" in os.environ:
        environment["HALDIR_FR0015_GH"] = os.environ["HALDIR_FR0015_GH"]
    ...
    for command in commands:
        ...
        if type(returncode) is not int or returncode != 0:
            _fail("FR0015_CAPTURE_LOCAL_COMMAND")
        checks.append(
            {
                "argv": list(command),
                "returncode": 0,
    ...
    value = {
        "schema_version": "1.0.0",
        "protocol": "HALDIR_FR_0015_LOCAL_VALIDATION_V1",
    ...
    _write_exclusive(output, bridge.canonical_json_bytes(value))
    print("framework-recovery-fr-0015-capture: OK (local R validation; release NO_GO)")
...
def _verify_root(
    ...
    for line in raw.splitlines(keepends=True):
        try:
            ...
            _fail("FR0015_CAPTURE_TRUSTED_ROOT")
    ...
    if matches != [expected]:
        _fail("FR0015_CAPTURE_TRUSTED_ROOT_DRIFT")
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        repo = _repository()
        bridge = _load_module(repo / BRIDGE_PATH, "_haldir_fr0015_capture_bridge")
        protocol = _load_module(repo / PROTOCOL_PATH, "_haldir_fr0015_capture_protocol")
        ...
        print(f"framework-recovery-fr-0015-capture: {error}", file=sys.stderr)
        return 1
    return 0
