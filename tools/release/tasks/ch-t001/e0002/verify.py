...
TASK_ID = "CH-T001"
EPOCH = 2
BASELINE_COMMIT = "c713bad6cf1cfbc193079e70cd9b23c23a65ffa9"
...
EXPECTED_PRODUCT_SHA256 = {
    ...
    PRODUCT_TESTS: "c2a96b114d518f06e0f8387a0921551e1522c8a948c3b077e4f444c981084d32",
}
...
REGISTERED_TESTS_PATH = "tools/release/tasks/ch-t001/e0002/test_verify.py"
FREEZE_PATH = "release/0.9.0/current-head/tasks/ch-t001/e0002/freeze.json"
QUALIFICATION_PATH = "release/0.9.0/current-head/tasks/ch-t001/e0002/qualification.json"
ACTIVATION_PATH = "release/0.9.0/current-head/tasks/ch-t001/e0002/activation.json"
RECEIPT_PATH = "release/0.9.0/current-head/tasks/ch-t001/e0002/verifier-receipt.json"
...
EXPECTED_SOURCE_PATHS = 353
EXPECTED_IMPLEMENTATION_PATHS = 356
...
MAX_CI_LOG_ARCHIVE_BYTES = 2 * 1024 * 1024
MAX_CI_LOG_EXPANDED_BYTES = 4 * 1024 * 1024
...
RAW_ACTIVATION_REQUIREMENTS = {
    "CH-T001-A05": {
        "path": (
            "release/0.9.0/current-head/tasks/ch-t001/e0002/"
            "activation-evidence/full-locked-ci-attempt-logs.zip"
        ),
        "kind": "FULL_LOCKED_CI_LOG_ARCHIVE",
        "max_bytes": MAX_CI_LOG_ARCHIVE_BYTES,
    }
}
...
def _require_wave_gate_result(command: dict[str, Any]) -> None:
    ...
    _require(
        pass_names == list(P0R_GATE_NAMES)
        and not fail_lines
...
def _retained_ci_file_payload(
    ...
    *,
    ...
    outer_completed: str,
    previous_completed: str,
    evidence_started: str,
    evidence_completed: str,
) -> tuple[bytes, str]:
    record = _exact_keys(
        value,
        {
        ...
        "ACTIVATION_CI_FILE_RECORD_KEYS",
    )
    expected_headers = [
        f"Accept: {GH_ACCEPT_HEADER}",
        f"X-GitHub-Api-Version: {GH_API_VERSION}",
    ...
    started = _timestamp(record["started_at_utc"], "ACTIVATION_CI_FILE_STARTED")
    completed = _timestamp(
        record["completed_at_utc"], "ACTIVATION_CI_FILE_COMPLETED"
    )
    _require(
        record["artifact_evidence_id"] == evidence_id
        ...
        and 0 < len(payload) <= maximum
        and started == evidence_started
        and completed == evidence_completed
        and outer_started
        <= previous_completed
        <= started
        <= completed
        <= outer_completed,
        "ACTIVATION_CI_FILE_RECORD_BINDING",
    )
    return payload, completed
...
def _validate_typed_activation_bodies(
    *,
    ...
    _require(
        [item["id"] for item in requirements] == expected_requirement_ids,
        "ACTIVATION_REQUIREMENT_ORDER",
    )
    for evidence_id, expected in RAW_ACTIVATION_REQUIREMENTS.items():
        requirement = next(
            item for item in requirements if item["id"] == evidence_id
        )
        _require(
            _strict_equal(
                requirement,
                {"id": evidence_id, **expected},
            ),
            "ACTIVATION_RAW_REQUIREMENT_BINDING",
        )
    ...
    for requirement, record in zip(requirements, records, strict=True):
        ...
        started = _timestamp(record["started_at_utc"], "ACTIVATION_STARTED")
        completed = _timestamp(record["completed_at_utc"], "ACTIVATION_COMPLETED")
        expected_file = _central_file_record(
            path, activation_entries[path], payload
        )
        _require(
        ...
        times[evidence_id] = (started, completed)
        file_records[evidence_id] = expected_file
        if evidence_id in RAW_ACTIVATION_REQUIREMENTS:
            continue
        _require(len(payload) <= MAX_JSON_BYTES, "ACTIVATION_JSON_FILE_BOUND")
        document = _canonical_json(payload, evidence_id)
        _exact_keys(
            document,
    ...
    _require(
        isinstance(retained_records, list) and len(retained_records) == 2,
    ...
    log_evidence_id = "CH-T001-A05"
    log_path = RAW_ACTIVATION_REQUIREMENTS[log_evidence_id]["path"]
    log_payload, _capture_completed = _retained_ci_file_payload(
        hosted["log_archive_capture"],
        evidence_id=log_evidence_id,
        expected_file=file_records[log_evidence_id],
        payload=activation_payloads[log_path],
        ...
        outer_completed=times["CH-T001-A03"][1],
        previous_completed=capture_completed,
        evidence_started=times[log_evidence_id][0],
        evidence_completed=times[log_evidence_id][1],
    )
