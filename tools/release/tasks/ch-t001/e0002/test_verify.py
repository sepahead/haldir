...
class RegisteredVerifierTests(unittest.TestCase):
    ...
    def test_technique_mutation(self):
        ...
        mutations = {
            ...
            "CH-T001-N02": ("i_count", module.EXPECTED_IMPLEMENTATION_PATHS + 1),
            "CH-T001-N03": ("rows", module.EXPECTED_IMPLEMENTATION_PATHS - 1),
    ...
    def test_n02_extra_implementation_path_is_rejected(self):
        module = verifier_module()
        facts = accepted_facts()
        facts["i_count"] = module.EXPECTED_IMPLEMENTATION_PATHS + 1
        with self.assertRaises(module.VerificationError):
    ...
    def test_independent_row_classification_matches_frozen_product_semantics(self):
        ...
        cases = {
            ...
            "tools/release/tasks/ch-t001/e0002/test_verify.py": "TEST",
    ...
    def test_n16_typed_evidence_and_review_bodies_are_accepted(self):
        ...
        for aliased_epoch in (True, float(module.EPOCH)):
        ...
        wave_stdout = "\n".join(
            [
                ...
                *(f"  PASS: {name}" for name in module.P0R_GATE_NAMES),
                module.P0R_GATE_SUMMARY,
                module.P0R_GATE_EPILOGUE,
            ]
        ) + "\n"
        module._require_wave_gate_result(
            {
                "stdout": wave_stdout,
                "stderr": "Finished test profile in 0.01s\n",
            }
        )
    ...
    def test_n16_empty_placeholder_or_cross_record_evidence_is_rejected(self):
        ...
        first_pass = f"  PASS: {module.P0R_GATE_NAMES[0]}"
        last_pass = f"  PASS: {module.P0R_GATE_NAMES[-1]}"
        mutations = (
            valid_wave.replace("  PASS: rustfmt", "  FAIL: rustfmt", 1),
            valid_wave.replace(
                module.P0R_GATE_SUMMARY,
                "P0-R exit gate: 29 passed, 0 failed",
                1,
            ),
            valid_wave.replace(
                module.P0R_GATE_EPILOGUE,
            ...
            valid_wave.replace(
                module.P0R_GATE_SUMMARY,
                f"{module.P0R_GATE_SUMMARY}\n{module.P0R_GATE_SUMMARY}",
                1,
            ),
            valid_wave.replace(
                module.P0R_GATE_SUMMARY,
                f"embedded {module.P0R_GATE_SUMMARY}",
                1,
            ),
            valid_wave.replace(
                module.P0R_GATE_EPILOGUE,
                f"embedded {module.P0R_GATE_EPILOGUE}",
                1,
            ),
            valid_wave.replace(f"{module.P0R_GATE_SUMMARY}\n", "", 1).replace(
                first_pass,
                f"{module.P0R_GATE_SUMMARY}\n{first_pass}",
                1,
            ),
            valid_wave.replace(f"{last_pass}\n", "", 1).replace(
                module.P0R_GATE_SUMMARY,
                f"{module.P0R_GATE_SUMMARY}\n{last_pass}",
                1,
            ),
            valid_wave.replace(
                ...
                (
                    f"{module.P0R_GATE_SUMMARY}\n"
                    "arbitrary post-summary text\n"
                    f"{module.P0R_GATE_EPILOGUE}"
                ),
                1,
            ),
            valid_wave.replace(
                first_pass,
                f"P0-R exit gate: 31 passed, 0 failed\n{first_pass}",
                1,
            ),
            valid_wave.replace(
                first_pass,
            ...
            valid_wave.replace(
                first_pass,
                f"All offline P0-R gates passed.\n{first_pass}",
                1,
            ),
        )
        for mutation in mutations:
            with self.subTest(mutation=mutation[-160:]):
                with self.assertRaises(module.VerificationError):
                    module._require_wave_gate_result(
                        {"stdout": mutation, "stderr": "successful cargo diagnostics\n"}
                    )
        ...
        self.assertEqual(
            module.EXPECTED_PRODUCT_SHA256,
            {
                ...
                module.PRODUCT_TESTS: (
                    "c2a96b114d518f06e0f8387a0921551e1522c8a948c3b077e4f444c981084d32"
                ),
            },
        )
    ...
    def test_activation_schemas_and_exact_safe_directory_are_frozen(self):
        module = verifier_module()
        self.assertEqual(
            module.ACTIVATION_SCHEMA_IDS,
            {
                "CH-T001-A01": "haldir.ch-t001.subsystem-gate.v1",
                "CH-T001-A02": "haldir.ch-t001.wave-gate.v2",
                "CH-T001-A03": "haldir.ch-t001.full-locked-ci.v2",
                "CH-T001-A04": (
                    "haldir.ch-t001.downstream-conformance-disposition.v1"
                ),
            },
        )
        self.assertEqual(tuple(module.RAW_ACTIVATION_REQUIREMENTS), ("CH-T001-A05",))
        self.assertEqual(
            module.RAW_ACTIVATION_REQUIREMENTS["CH-T001-A05"]["max_bytes"],
            2 * 1024 * 1024,
        )
        ...
        self.assertEqual(
            module.P0R_GATE_ARGV[5:7],
            ("CARGO_TERM_COLOR=never", "TERM=dumb"),
        )
        repo = Path(__file__).resolve().parents[5]
        gate_source = (repo / "tools/p0r-exit-gate.sh").read_text(encoding="utf-8")
        gate_names = [
            line.split('"', 2)[1]
            for line in gate_source.splitlines()
            if line.startswith('run "')
        ]
        self.assertEqual(gate_names, list(module.P0R_GATE_NAMES))
        self.assertIn(module.P0R_GATE_EPILOGUE, gate_source)
    ...
    def test_retained_ci_records_and_logs_are_byte_bound(self):
        module = verifier_module()
        inline_payload = b'{"ok":true}'
        inline_record = {
            "bytes": len(inline_payload),
            "capture_argv": [
            ...
            "sha256": hashlib.sha256(inline_payload).hexdigest(),
            "source_url": "https://api.github.com/example/run",
            "started_at_utc": "2026-07-15T12:00:01Z",
            "stderr": "",
        ...
        self.assertEqual(
            ...
            (inline_payload, "2026-07-15T12:00:02Z"),
        )
        inline_record["sha256"] = "0" * 64
        with self.assertRaises(module.VerificationError):
            module._retained_ci_payload(
                inline_record,
                kind="RUN_API_JSON",
        ...
        artifact_path = module.RAW_ACTIVATION_REQUIREMENTS["CH-T001-A05"]["path"]
        artifact_entry = {
            "mode": "100644",
            "type": "blob",
            "oid": module._git_blob_id(payload),
        }
        artifact_file = module._central_file_record(
            artifact_path, artifact_entry, payload
        )
        self.assertNotIn("lines", artifact_file)
        record = {
            "artifact_evidence_id": "CH-T001-A05",
            "capture_argv": [
                module.GH_EXECUTABLE,
                "api",
                "--hostname",
        ...
        self.assertEqual(
            module._retained_ci_file_payload(
                record,
                evidence_id="CH-T001-A05",
                ...
                outer_completed="2026-07-15T12:00:03Z",
                previous_completed="2026-07-15T12:00:00Z",
                evidence_started="2026-07-15T12:00:01Z",
                evidence_completed="2026-07-15T12:00:02Z",
            ),
            (payload, "2026-07-15T12:00:02Z"),
        )
        record["file"] = dict(artifact_file, sha256="0" * 64)
        with self.assertRaises(module.VerificationError):
            module._retained_ci_file_payload(
                record,
                evidence_id="CH-T001-A05",
        ...
        generator = random.Random(0xA05C1)
        large_stream = io.BytesIO()
        with zipfile.ZipFile(
            large_stream, "w", zipfile.ZIP_DEFLATED
        ) as archive:
            for name in module.CI_JOB_NAMES:
                archive.writestr(
                    f"0_{name}.txt",
                    bytes(generator.getrandbits(8) for _ in range(30_000)),
                )
        large_payload = large_stream.getvalue()
        self.assertGreater(len(large_payload), 128 * 1024)
        self.assertLess(len(large_payload), module.MAX_CI_LOG_ARCHIVE_BYTES)
        module._validate_ci_log_archive(large_payload)

        expanded_stream = io.BytesIO()
        with zipfile.ZipFile(
            expanded_stream, "w", zipfile.ZIP_DEFLATED
        ) as archive:
            archive.writestr(
                "0_build-test.txt",
                b"x" * (module.MAX_CI_LOG_EXPANDED_BYTES + 1),
            )
            for name in module.CI_JOB_NAMES[1:]:
                archive.writestr(f"0_{name}.txt", b"complete\n")
        with self.assertRaises(module.VerificationError):
            module._validate_ci_log_archive(expanded_stream.getvalue())
    ...
    def test_ledger_parser_enforces_field_limits_unicode_nfc_and_no_controls(self):
        ...
        self.assertEqual(
            len(module._parse_ledger(render())), module.EXPECTED_IMPLEMENTATION_PATHS
        )
