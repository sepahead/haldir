...
# Exact predecessor of the CH-T002 e0002 F transition. The classification audit
# keeps its original signed-current-A identity in CLASSIFICATION_AUDIT_COMMIT.
# No I/C/D identity belongs in this F-frozen verifier.
BASELINE_COMMIT = "7c47d3a994624046f999ff3016a8fed2d8f70353"
...
EXPECTED_CLASSIFICATION_OVERRIDE_PATHS = 75
EXPECTED_CLASSIFICATION_OVERRIDE_FIELDS = 93
EXPECTED_GITIGNORE_OVERRIDE = {
    "after": "YES",
    "before": "NO",
    "field": "public_surface",
    "path": ".gitignore",
    "rule_id": "PUB_YES_BUILD_OR_DEPLOYMENT_AFFECTED_SURFACE",
    "source_basis": CLASSIFICATION_OVERRIDE_SOURCE_BASIS,
}
...
PRIOR_TASK_ROOT = "release/0.9.0/current-head/tasks/ch-t002/e0001"
PRIOR_FREEZE_PATH = f"{PRIOR_TASK_ROOT}/freeze.json"
PRIOR_VERIFIER_PATH = "tools/release/tasks/ch-t002/e0001/verify.py"
PRIOR_REGISTERED_TESTS_PATH = "tools/release/tasks/ch-t002/e0001/test_verify.py"
DEFECT_EVIDENCE_PATH = (
    "release/0.9.0/current-head/tasks/revocations/R0002/"
    "classification-public-surface-conflict.json"
)
VERIFIER_PATH = "tools/release/tasks/ch-t002/e0002/verify.py"
REGISTERED_TESTS_PATH = "tools/release/tasks/ch-t002/e0002/test_verify.py"
TASK_ROOT = "release/0.9.0/current-head/tasks/ch-t002/e0002"
...
EXPECTED_COMMIT_SUBJECTS = {
    "F": "release: refreeze CH-T002 file-review coverage",
...
EXPECTED_F_EXTENSION_CLASSIFICATIONS = {
    ...
    PRIOR_REGISTERED_TESTS_PATH: {
        **_COMMON_F_EXTENSION_CLASSIFICATION,
        "path": PRIOR_REGISTERED_TESTS_PATH,
        "rule_ids": [
            "PUB_NO_INTERNAL_ASSURANCE_OR_IMPLEMENTATION_ARTIFACT",
    ...
    DEFECT_EVIDENCE_PATH: {
        **_COMMON_F_EXTENSION_CLASSIFICATION,
        "path": DEFECT_EVIDENCE_PATH,
        "rule_ids": [
            "PUB_NO_INTERNAL_ASSURANCE_OR_IMPLEMENTATION_ARTIFACT",
            "SEC_YES_DECISIVE_SIGNED_REVOCATION_CAUSE_EVIDENCE",
            "SCI_NO_NO_MODEL_CONTROL_NUMERIC_OR_RESEARCH_SEMANTICS",
            "AUTH_YES_MANDATORY_CURRENT_HEAD_RELEASE_STATE_FLOOR",
            "GEN_NO_AUTHORED_IMPORTED_OR_PRIMARY_RECORD",
            "LIC_REPOSITORY_DUAL_LICENSE_AND_GIT_PROVENANCE",
...
def unresolved_lifecycle_fields() -> tuple[str, ...]:
    ...
    if HEX40.fullmatch(CLASSIFICATION_AUDIT_COMMIT) is None:
        unresolved.append("CLASSIFICATION_AUDIT_COMMIT")
...
def _validate_freeze(
    ...
    _expected_qualification_argv(freeze, "1" * 40, "2" * 40)
...
def _validate_classification_contract(
    ...
    if EXPECTED_GITIGNORE_OVERRIDE is not None:
        _require(
            EXPECTED_GITIGNORE_OVERRIDE in rows,
            "CLASSIFICATION_GITIGNORE_PUBLIC_SURFACE_OVERRIDE",
        )
...
def _expected_qualification_argv(
    ...
    packet = [
        "--repo", ".", "--implementation-commit", implementation_commit,
        "--ledger", LEDGER_PATH, "--overlay", OVERLAY_PATH,
        "--output-dir", "RETAINED_PACKET_DIRECTORY",
        "--reviewer-registry", FREEZE_PATH,
    ]
    required = (
        ("PRODUCT_TESTS", [*python, PRODUCT_TESTS]),
        ("PACKET_RENDER", [*python, PRODUCT_TOOL, "render", *packet]),
        ("PACKET_VERIFY", [*python, PRODUCT_TOOL, "verify", *packet]),
        ("REGISTERED_TESTS", [*python, REGISTERED_TESTS_PATH]),
        (
            "INDEPENDENT_RECONCILIATION",
            [*python, VERIFIER_PATH, "--repo", ".", "--freeze-commit", freeze_commit,
             "--implementation-commit", implementation_commit, "--inventory-only"],
        ),
        ("NEGATIVE_VECTORS", [*python, REGISTERED_TESTS_PATH, "-k", "reject"]),
        ("TECHNIQUE_ANALYSIS", [*python, REGISTERED_TESTS_PATH, "-k", "technique_"]),
    )
    by_phase = {phase: argv for phase, argv in task_commands}
    _require(
        len(by_phase) == len(task_commands) == len(required)
        and all(_strict_equal(by_phase.get(phase), argv) for phase, argv in required),
