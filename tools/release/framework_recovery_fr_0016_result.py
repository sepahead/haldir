#!/usr/bin/env python3
"""Emit the canonical epoch-17 hosted-check result artifact.
...
def _file_record(repo: Path, commit: str, path: str) -> dict[str, Any]:
    ...
    if not raw.endswith(b"\0") or raw.count(b"\0") != 1:
        _fail("FR0016_RESULT_MATERIAL:" + path)
    try:
...
def build_result(
    ...
    *,
    ...
    if contract is None:
        _fail("FR0016_RESULT_WORKFLOW")
    if _exact_environment(environment, "GITHUB_ACTIONS") != "true":
        _fail("FR0016_RESULT_NOT_ACTIONS")
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        repo = Path(
            _git(Path.cwd(), "rev-parse", "--show-toplevel").decode("utf-8").strip()
        ).resolve(strict=True)
        result = build_result(
            repo, workflow=arguments.workflow, environment=dict(os.environ)
        )
        _write_exclusive(
            arguments.output,
            canonical_json_bytes(result),
            expected_name=(
                f"epoch-17-{arguments.workflow}-result-attempt-"
        ...
        print(f"framework-recovery-fr-0016-result: {error}", file=sys.stderr)
        return 1
    print(
        "framework-recovery-fr-0016-result: OK "
