#!/usr/bin/env python3
"""Capture bounded FR-0011 evidence using exact GitHub identities.
...
MAX_PROVIDER_STDERR = 64 * 1024
BRIDGE_PATH = "tools/release/verify-framework-recovery-fr-0011.py"
PROTOCOL_PATH = "tools/release/framework_recovery_fr_0011.py"
...
def _git(repo: Path, *arguments: str, limit: int = MAX_JSON_BYTES) -> bytes:
    ...
    if completed.returncode != 0 or completed.stderr or len(completed.stdout) > limit:
        _fail("FR0011_CAPTURE_GIT:" + (arguments[0] if arguments else "missing"))
...
def _repository() -> Path:
    ...
    try:
        ...
        _fail("FR0011_CAPTURE_REPOSITORY")
...
def _gh(
    ...
    *,
    ...
    returncode, stdout, stderr = bridge._run_bounded(
        ...
        output_limit=limit + MAX_PROVIDER_STDERR,
    )
    if returncode != 0 or len(stdout) > limit or len(stderr) > MAX_PROVIDER_STDERR:
        _fail("FR0011_CAPTURE_GH:" + (arguments[0] if arguments else "missing"))
    return stdout


def _json_bytes(raw: bytes, label: str) -> dict[str, Any]:
    if not raw or len(raw) > MAX_JSON_BYTES or b"\0" in raw:
        _fail("FR0011_CAPTURE_JSON:" + label)
    try:
...
def _logical_path(repo: Path, path: Path) -> str:
    ...
    try:
        ...
        _fail("FR0011_CAPTURE_OUTPUT_SCOPE")
...
def _stage_paths(bridge: ModuleType, stage: str, workflow: str) -> tuple[str, str, str]:
    ...
    _fail("FR0011_CAPTURE_STAGE")


def _run_fields(*, epoch12: bool, attempt: bool) -> str:
    fields = [
        "attempt",
        "conclusion",
        "createdAt",
...
def _capture_hosted(
    ...
    if any(path.exists() or path.is_symlink() for path in output_paths):
        _fail("FR0011_CAPTURE_OUTPUT_EXISTS")
    if arguments.run_id < 1:
        _fail("FR0011_CAPTURE_RUN_ID")
    ...
    ordinary_fields = _run_fields(epoch12=True, attempt=False)
    ...
    if (
        ...
        or not 1 <= attempt_number <= protocol.MAX_EPOCH12_RUN_ATTEMPT
    ):
        _fail("FR0011_CAPTURE_ATTEMPT")
    attempt_fields = _run_fields(epoch12=True, attempt=True)
    attempt = _json_bytes(
        _gh(
            bridge,
            executable,
    ...
    artifact_name = f"epoch-12-{workflow}-result-attempt-{attempt_number}.json"
    listing_endpoint = (
    ...
    if (
        set(listing) != {"artifacts", "total_count"}
        or listing["total_count"] != 1
        or not isinstance(listing["artifacts"], list)
        or len(listing["artifacts"]) != 1
        or not isinstance(listing["artifacts"][0], dict)
    ):
        _fail("FR0011_CAPTURE_ARTIFACT_UNIQUENESS")
    artifact = listing["artifacts"][0]
    artifact_id = artifact.get("id")
    if type(artifact_id) is not int or artifact_id < 1:
        _fail("FR0011_CAPTURE_ARTIFACT_ID")
    ...
    if artifact_by_id != artifact:
        _fail("FR0011_CAPTURE_ARTIFACT_IDENTITY")
    result_payload = _gh(
        bridge,
        executable,
        (
    ...
    if not result_payload.endswith(b"\n"):
        _fail("FR0011_CAPTURE_DIRECT_ARTIFACT")
    materials = [
    ...
    with tempfile.TemporaryDirectory(prefix="haldir-fr0011-download-") as name:
        root = Path(name)
        ...
        if len(bundles) != 1:
            _fail("FR0011_CAPTURE_ATTESTATION_FILE")
        bundle_payload = bundles[0].read_bytes()
    if not 1 <= len(bundle_payload) <= MAX_BUNDLE_BYTES:
        _fail("FR0011_CAPTURE_ATTESTATION_BOUND")
    ...
    capture = {
        ...
        "protocol": "HALDIR_FR_0011_HOSTED_RESULT_CAPTURE_V1",
        "workflow": workflow,
        "subject_commit": subject_commit,
        "subject_tree": subject_tree,
        "expected_ref": "refs/heads/main",
    ...
    print(
        "framework-recovery-fr-0011-capture: OK "
...
def _capture_c5(
    ...
    workflow = arguments.workflow
    paths = bridge.C5_CI_PATHS if workflow == "ci" else bridge.C5_FORMAL_PATHS
    outputs = tuple(repo / path for path in paths)
    if any(path.exists() or path.is_symlink() for path in outputs):
        _fail("FR0011_CAPTURE_OUTPUT_EXISTS")
    executable, _version = bridge._trusted_gh()
    started = _now()
    ordinary_fields = _run_fields(epoch12=False, attempt=False)
    ordinary = _json_bytes(
        _gh(
            bridge,
            executable,
    ...
    if type(attempt_number) is not int or attempt_number < 1:
        _fail("FR0011_CAPTURE_ATTEMPT")
    attempt_fields = _run_fields(epoch12=False, attempt=True)
    attempt = _json_bytes(
        _gh(
            bridge,
            executable,
    ...
    archive = _gh(
        ...
        (
            ...
            "--method",
            "GET",
            f"repos/{REPOSITORY}/actions/runs/{arguments.run_id}/attempts/"
            f"{attempt_number}/logs",
        ),
        cwd=repo,
        limit=MAX_ARCHIVE_BYTES,
    )
    run = protocol.validate_c5_run_documents(
        ordinary,
        attempt,
        workflow=workflow,
    ...
    archive_result = (
        protocol.validate_c5_ci_archive(
            archive, jobs=run["jobs"], suite_counts=suite_counts
        )
        if workflow == "ci"
        else protocol.validate_c5_formal_archive(archive, jobs=run["jobs"])
    )
    fields = _run_fields(epoch12=False, attempt=False)
    attempt_fields = _run_fields(epoch12=False, attempt=True)
    completed = _now()
    value = {
        "schema_version": "1.0.0",
        "protocol": protocol.C5_ARCHIVE_PROTOCOL,
        "workflow": workflow,
        "subject_commit": bridge.PARENT_PARENT,
        "ordinary": ordinary,
        "attempt_metadata": attempt,
        ...
        "capture": {
            "tool": bridge.file_record(
                repo, arguments.repair_commit, bridge.CAPTURE_PATH
            ),
            "metadata_command": (
                f"gh run view {arguments.run_id} --repo {REPOSITORY} --json {fields}"
            ),
            "attempt_command": (
                f"gh run view {arguments.run_id} --repo {REPOSITORY} "
                f"--attempt {attempt_number} --json {attempt_fields}"
            ),
            "archive_command": (
                "gh api --method GET "
                f"repos/{REPOSITORY}/actions/runs/{arguments.run_id}/attempts/"
    ...
    protocol.validate_c5_hosted_capture(
        value,
        archive,
        workflow=workflow,
        subject_commit=bridge.PARENT_PARENT,
    ...
    _write_exclusive(outputs[1], archive)
    _write_exclusive(outputs[0], bridge.canonical_json_bytes(value))
    print(
        "framework-recovery-fr-0011-capture: OK "
        f"(c5 {workflow}; run {run['run_id']}; diagnostic only)"
    )
...
def _capture_branch_protection(
    ...
    if observed != arguments.qualification_commit:
        _fail("FR0011_CAPTURE_PROTECTION_HEAD")
    ...
    value = {
        "schema_version": "1.0.0",
        "protocol": "HALDIR_FR_0011_BRANCH_PROTECTION_CAPTURE_V1",
...
def _review_contract(
    bridge: ModuleType, review_id: str
) -> tuple[str, tuple[str, str, str], list[dict[str, Any]]]:
    if review_id == "FR-0011-R01":
        model = "claude-fable-5"
        paths = bridge.DESIGN_REVIEW_PATHS
    elif review_id == "FR-0011-R02":
        model = "claude-opus-5"
        paths = bridge.IMPLEMENTATION_REVIEW_PATHS
    else:
        _fail("FR0011_CAPTURE_REVIEW_ID")
    findings = [
        {
            "id": "F001",
        ...
        },
        {
            ...
            "affected_paths": [bridge.BRIDGE_PATH, bridge.PLAN_PATH],
        },
        {
            "id": "F003",
            "summary": "Exact model response bytes are retained and reparsed.",
            "affected_paths": [bridge.CAPTURE_PATH, paths[2]],
        },
    ]
    return model, paths, findings
...
def _review_material(
    ...
    *,
    ...
    model, _paths, findings = _review_contract(bridge, review_id)
    plan_payload = bridge._file(repo, repair_commit, bridge.PLAN_PATH)
    patch_payload = bridge._core_patch(repo, repair_commit)
    gate_payload = bridge._file(repo, repair_commit, bridge.GATE_PATH)
...
def _review_request(
    ...
    _manifest, request = _review_material(
        repo,
        bridge,
        protocol,
    ...
    if output.exists() or output.is_symlink():
        _fail("FR0011_CAPTURE_OUTPUT_EXISTS")
    _write_exclusive(output, request, mode=0o600)
    print(
        "framework-recovery-fr-0011-capture: OK "
        f"({arguments.review_id} request; sha256 "
...
def _capture_review(
    ...
    model, paths, _findings = _review_contract(bridge, arguments.review_id)
    outputs = tuple(repo / path for path in paths)
    if any(path.exists() or path.is_symlink() for path in outputs):
        _fail("FR0011_CAPTURE_OUTPUT_EXISTS")
    manifest, request = _review_material(
        repo,
        bridge,
        protocol,
        review_id=arguments.review_id,
    ...
    provider_response = _read_regular_bounded(
        arguments.provider_response,
        limit=protocol.MAX_PROVIDER_RESPONSE_BYTES,
        label="FR0011_CAPTURE_PROVIDER_RESPONSE",
    )
    response = protocol.extract_provider_review_response_bytes(
    ...
    capture = {
        ...
        "provider_response_record": _blob_record(paths[1], provider_response),
        "response_record": _blob_record(paths[2], response),
        "parsed_outcome": outcome,
        "provider_provenance_independently_attested": False,
        "review_authority_conferred": False,
    }
    _write_exclusive(outputs[1], provider_response)
    _write_exclusive(outputs[2], response)
    _write_exclusive(outputs[0], bridge.canonical_json_bytes(capture))
    print(
        "framework-recovery-fr-0011-capture: OK "
        f"({arguments.review_id}; {outcome['verdict']}; "
        "provider provenance not independently attested)"
    )
...
def _assemble_signed_record(
    ...
    unsigned = _read_regular_bounded(
        arguments.unsigned,
        limit=MAX_JSON_BYTES,
        label="FR0011_CAPTURE_UNSIGNED_RECORD",
    )
    signature_payload = _read_regular_bounded(
        arguments.signature,
        limit=16 * 1024,
        label="FR0011_CAPTURE_RECORD_SIGNATURE",
    )
    try:
        ...
        _fail("FR0011_CAPTURE_UNSIGNED_RECORD")
...
def _capture_local(
    ...
    if (
        ...
        _fail("FR0011_CAPTURE_LOCAL_HEAD")
    ...
    if "HALDIR_FR0011_GH" in os.environ:
        environment["HALDIR_FR0011_GH"] = os.environ["HALDIR_FR0011_GH"]
    ...
    for command in commands:
        ...
        if returncode != 0:
            _fail("FR0011_CAPTURE_LOCAL_COMMAND")
        checks.append(
            {
                "argv": list(command),
                "returncode": 0,
    ...
    value = {
        "schema_version": "1.0.0",
        "protocol": "HALDIR_FR_0011_LOCAL_VALIDATION_V1",
    ...
    _write_exclusive(output, bridge.canonical_json_bytes(value))
    print("framework-recovery-fr-0011-capture: OK (local R validation; release NO_GO)")
...
def _verify_root(
    ...
    for line in raw.splitlines(keepends=True):
        try:
            ...
            _fail("FR0011_CAPTURE_TRUSTED_ROOT")
    ...
    if matches != [expected]:
        _fail("FR0011_CAPTURE_TRUSTED_ROOT_DRIFT")
...
def _parser() -> argparse.ArgumentParser:
    ...
    c5.add_argument("--workflow", choices=("ci", "formal"), required=True)
    c5.add_argument("--run-id", type=int, required=True)
    c5.add_argument("--repair-commit", required=True)
    protection = subparsers.add_parser("branch-protection")
    protection.add_argument("--qualification-commit", required=True)
    request = subparsers.add_parser("review-request")
    request.add_argument(
        "--review-id", choices=("FR-0011-R01", "FR-0011-R02"), required=True
    )
    request.add_argument("--repair-commit", required=True)
    request.add_argument("--output", type=Path, required=True)
    review = subparsers.add_parser("review-capture")
    review.add_argument(
        "--review-id", choices=("FR-0011-R01", "FR-0011-R02"), required=True
    )
    review.add_argument("--repair-commit", required=True)
    review.add_argument("--provider-response", type=Path, required=True)
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        repo = _repository()
        bridge = _load_module(repo / BRIDGE_PATH, "_haldir_fr0011_capture_bridge")
        protocol = _load_module(repo / PROTOCOL_PATH, "_haldir_fr0011_capture_protocol")
        if arguments.command == "hosted":
            _capture_hosted(repo, bridge, protocol, arguments)
        elif arguments.command == "c5":
            _capture_c5(repo, bridge, protocol, arguments)
        elif arguments.command == "branch-protection":
            _capture_branch_protection(repo, bridge, arguments)
        elif arguments.command == "review-request":
            _review_request(repo, bridge, protocol, arguments)
        elif arguments.command == "review-capture":
            _capture_review(repo, bridge, protocol, arguments)
        ...
        print(f"framework-recovery-fr-0011-capture: {error}", file=sys.stderr)
        return 1
    return 0
