#!/usr/bin/env python3
"""Pure validation primitives for the FR-0017 epoch-18 recovery.
...
RESULT_PROTOCOL = "HALDIR_EPOCH_18_HOSTED_RESULT_V1"
DISPATCHER_STEP_NAME = "Verify epoch-18 recovery primitives for current event"
...
EPOCH18_CI_JOB_NAMES = CI_JOB_NAMES | frozenset({"attest-ci-audit-result"})
EPOCH18_FORMAL_JOB_NAMES = FORMAL_JOB_NAMES | frozenset({"attest-formal-audit-result"})
RESULT_CONTRACT = {
    "ci": {
...
EPOCH18_ORDINARY_RUN_FIELDS = ORDINARY_RUN_FIELDS | {
    "number",
    "workflowDatabaseId",
}
EPOCH18_ATTEMPT_RUN_FIELDS = ATTEMPT_RUN_FIELDS | {"number"}
...
class RecoveryProtocolError(ValueError):
    """One fail-closed FR-0017 validation error."""
...
def _require_dict(value: Any, fields: frozenset[str], label: str) -> dict[str, Any]:
    if not isinstance(value, dict) or set(value) != fields:
        _fail("FR0017_FIELDS:" + label)
    return value


def _bounded_int(value: Any, *, minimum: int, maximum: int, label: str) -> int:
    if type(value) is not int or not minimum <= value <= maximum:
        _fail("FR0017_INTEGER:" + label)
    return value
    ...
    *,
    ...
    for index, raw in enumerate(jobs):
        ...
        if (
            ...
            _fail("FR0017_JOB:" + f"{label}.{index}")
        ...
        if started > completed or started < run_start or completed > run_end:
            _fail("FR0017_JOB_TIME:" + f"{label}.{index}")
        steps = [
            _validate_step(
                step,
                run_start=started,
        ...
        if not steps or len(numbers) != len(set(numbers)) or numbers != sorted(numbers):
            _fail("FR0017_JOB_STEPS:" + f"{label}.{index}")
        result[name] = {
    ...
    if set(result) != expected_names:
        _fail("FR0017_JOB_SET:" + label)
    return result


def validate_epoch18_run_documents(
    ordinary: Any,
    attempt: Any,
    *,
    workflow: str,
    ...
    """Validate a successful epoch-18 run, including its OIDC attestation job."""
    ...
    if (
        ...
        _fail("FR0017_EPOCH18_RUN_IDENTITY")
    ordinary = _require_dict(ordinary, EPOCH18_ORDINARY_RUN_FIELDS, "epoch18.ordinary")
    attempt = _require_dict(attempt, EPOCH18_ATTEMPT_RUN_FIELDS, "epoch18.attempt")
    run_id = _bounded_int(
        ordinary["databaseId"],
        minimum=1,
        maximum=2**63 - 1,
        label="epoch18.run_id",
    )
    attempt_number = _bounded_int(
        attempt["attempt"],
        minimum=1,
        maximum=MAX_EPOCH18_RUN_ATTEMPT,
        label="epoch18.attempt_number",
    )
    ordinary_attempt = _bounded_int(
        ordinary["attempt"],
        minimum=1,
        maximum=MAX_EPOCH18_RUN_ATTEMPT,
        label="epoch18.ordinary_attempt",
    )
    attempt_run_id = _bounded_int(
        attempt["databaseId"],
        minimum=1,
        maximum=2**63 - 1,
        label="epoch18.attempt_run_id",
    )
    ...
    if (
        ...
        _fail("FR0017_EPOCH18_RUN_METADATA")
    for key in COMMON_RUN_FIELDS - {"createdAt", "jobs", "updatedAt", "url"}:
        if ordinary[key] != attempt[key]:
            _fail("FR0017_EPOCH18_RUN_ATTEMPT_MISMATCH:" + key)
    created = _parse_utc(ordinary["createdAt"], "epoch18.ordinary.created")
    ordinary_updated = _parse_utc(ordinary["updatedAt"], "epoch18.ordinary.updated")
    attempt_created = _parse_utc(attempt["createdAt"], "epoch18.attempt.created")
    attempt_started = _parse_utc(attempt["startedAt"], "epoch18.attempt.started")
    attempt_updated = _parse_utc(attempt["updatedAt"], "epoch18.attempt.updated")
    if (
        ...
        _fail("FR0017_EPOCH18_RUN_CHRONOLOGY")
    expected_jobs = (
        EPOCH18_CI_JOB_NAMES if workflow == "ci" else EPOCH18_FORMAL_JOB_NAMES
    )
    ordinary_jobs = _validate_jobs(
        ordinary["jobs"],
        expected_names=expected_jobs,
        expected_run_id=run_id,
        run_start=created,
        run_end=ordinary_updated,
        label="epoch18.ordinary.jobs",
    )
    attempt_jobs = _validate_jobs(
        ...
        label="epoch18.attempt.jobs",
    )
    if canonical_json_bytes(ordinary["jobs"]) != canonical_json_bytes(attempt["jobs"]):
        _fail("FR0017_EPOCH18_RUN_JOB_MISMATCH")
    if workflow == "ci":
        _require_dispatcher_step(ordinary_jobs, label="epoch18.ordinary")
        _require_dispatcher_step(attempt_jobs, label="epoch18.attempt")
    ...
    if (
        ...
        _fail("FR0017_EPOCH18_CURRENT_ATTEMPT_JOBS")
    return {
        "run_id": run_id,
...
def _validate_pull_request_step(
    value: Any,
    *,
    run_start: datetime,
    run_end: datetime,
    ...
    if (
        ...
        _fail("FR0017_PULL_REQUEST_STEP:" + label)
    started = _parse_utc(step["startedAt"], label + ".started")
    completed = _parse_utc(step["completedAt"], label + ".completed")
    tolerance = timedelta(seconds=TIMESTAMP_SKEW_SECONDS)
    if (
        ...
        _fail("FR0017_PULL_REQUEST_STEP_TIME:" + label)
    return {
        "number": number,
        "name": step["name"],
        "status": step["status"],
...
def _validate_pull_request_jobs(
    jobs: Any,
    *,
    workflow: str,
    expected_run_id: int,
    ...
    required_names = CI_JOB_NAMES if workflow == "ci" else FORMAL_JOB_NAMES
    attestation_name = (
        "attest-ci-audit-result" if workflow == "ci" else "attest-formal-audit-result"
    )
    expected_names = required_names | frozenset({attestation_name})
    if not isinstance(jobs, list) or len(jobs) != len(expected_names):
        _fail("FR0017_PULL_REQUEST_JOBS")
    result: dict[str, dict[str, Any]] = {}
    job_ids: set[int] = set()
    for index, raw in enumerate(jobs):
        label = f"pull_request.jobs.{index}"
        job = _require_dict(raw, JOB_FIELDS, label)
        name = job["name"]
        database_id = _bounded_int(
            job["databaseId"],
            minimum=1,
            maximum=2**63 - 1,
            label=label + ".database_id",
        )
        if (
            not isinstance(name, str)
            ...
            f"{expected_run_id}/job/{database_id}"
        ):
            _fail("FR0017_PULL_REQUEST_JOB:" + label)
        started = _parse_utc(job["startedAt"], label + ".started")
        completed = _parse_utc(job["completedAt"], label + ".completed")
        tolerance = timedelta(seconds=TIMESTAMP_SKEW_SECONDS)
        if (
            started < run_start - tolerance
            or completed > run_end + tolerance
            or completed + tolerance < started
        ):
            _fail("FR0017_PULL_REQUEST_JOB_TIME:" + label)
        if name == attestation_name:
            if job["conclusion"] != "skipped" or job["steps"] != []:
                _fail("FR0017_PULL_REQUEST_ATTESTATION_JOB")
            steps: list[dict[str, Any]] = []
        else:
            if job["conclusion"] != "success" or not isinstance(job["steps"], list):
                _fail("FR0017_PULL_REQUEST_JOB:" + label)
            steps = [
                _validate_pull_request_step(
                    step,
                    run_start=started,
                    run_end=completed,
            ...
            if (
                ...
                or not any(
                    ...
                    and step["name"].startswith("Run actions/checkout@")
                    for step in steps
                )
            ):
                _fail("FR0017_PULL_REQUEST_JOB_STEPS:" + label)
        result[name] = {
            "database_id": database_id,
        ...
        job_ids.add(database_id)
    if set(result) != expected_names:
        _fail("FR0017_PULL_REQUEST_JOB_SET")
    return result


def _require_dispatcher_step(
    ...
    *,
    ...
    if not isinstance(supply_chain, dict):
        _fail("FR0017_DISPATCHER_JOB:" + label)
    matches = [
        step
        for step in supply_chain.get("steps", [])
        if step.get("name") == DISPATCHER_STEP_NAME
    ]
    if (
        len(matches) != 1
        ...
        _fail("FR0017_DISPATCHER_STEP:" + label)


def validate_pull_request_run_document(
    value: Any,
    *,
    workflow: str,
    subject_commit: str,
    ...
    """Validate one successful PR workflow run and its skipped OIDC job."""
    ...
    if (
        ...
        or re.fullmatch(r"[A-Za-z0-9._/-]+", head_branch) is None
        or len(head_branch.encode("ascii")) > 255
        or head_branch.startswith("/")
        or head_branch.endswith("/")
        or ".." in head_branch
        or "//" in head_branch
    ):
        _fail("FR0017_PULL_REQUEST_RUN_IDENTITY")
    run = _require_dict(value, EPOCH18_ORDINARY_RUN_FIELDS, "pull_request.run")
    run_id = _bounded_int(
        run["databaseId"],
        minimum=1,
        maximum=2**63 - 1,
        label="pull_request.run_id",
    )
    attempt = _bounded_int(
        run["attempt"],
        minimum=1,
        maximum=MAX_EPOCH18_RUN_ATTEMPT,
        label="pull_request.attempt",
    )
    run_number = _bounded_int(
        run["number"],
        minimum=1,
        maximum=2**63 - 1,
        label="pull_request.run_number",
    )
    workflow_id = _bounded_int(
        run["workflowDatabaseId"],
        minimum=1,
        maximum=2**63 - 1,
        label="pull_request.workflow_id",
    )
    expected_url = f"https://github.com/{REPOSITORY}/actions/runs/{run_id}"
    if (
        workflow_id != WORKFLOW_DATABASE_IDS[workflow]
        or run["event"] != "pull_request"
        or run["headBranch"] != head_branch
        or run["headSha"] != subject_commit
        or run["status"] != "completed"
        or run["conclusion"] != "success"
        or run["url"] != expected_url
        or run["workflowName"] != workflow
    ):
        _fail("FR0017_PULL_REQUEST_RUN_METADATA")
    created = _parse_utc(run["createdAt"], "pull_request.created")
    updated = _parse_utc(run["updatedAt"], "pull_request.updated")
    if created > updated:
        _fail("FR0017_PULL_REQUEST_RUN_CHRONOLOGY")
    jobs = _validate_pull_request_jobs(
        run["jobs"],
        workflow=workflow,
        expected_run_id=run_id,
        run_start=created,
    ...
    if workflow == "ci":
        _require_dispatcher_step(jobs, label="pull_request")
    return {
        "run_id": run_id,
        "attempt": attempt,
        "run_number": run_number,
...
def _validate_file_record(value: Any, *, expected_path: str) -> dict[str, Any]:
    ...
    if (
        ...
        _fail("FR0017_FILE_RECORD:" + expected_path)
...
def validate_result_artifact(
    ...
    *,
    ...
    if (
        ...
        _fail("FR0017_RESULT_BOUND")
    try:
        value = json.loads(payload)
    except (UnicodeDecodeError, json.JSONDecodeError):
        _fail("FR0017_RESULT_JSON")
    if (
    ...
    if (
        ...
        _fail("FR0017_RESULT_REPOSITORY")
    ...
    if canonical_json_bytes(normalized_observed) != canonical_json_bytes(
        ...
        _fail("FR0017_RESULT_MATERIAL_DRIFT")
    expected_authority = {
...
def validate_artifact_metadata(
    ...
    *,
    ...
    if workflow not in RESULT_CONTRACT:
        _fail("FR0017_ARTIFACT_WORKFLOW")
    ...
    if (
        ...
        _fail("FR0017_ARTIFACT_IDENTITY")
    ...
    if (
        ...
        _fail("FR0017_ARTIFACT_CHRONOLOGY")
    ...
    if (
        ...
        _fail("FR0017_ARTIFACT_RUN_BINDING")
...
def _parse_attestation_bundle(payload: bytes) -> dict[str, Any]:
    if (
        ...
        _fail("FR0017_ATTESTATION_BUNDLE_BOUND")
    try:
        bundle = json.loads(payload)
    except (UnicodeDecodeError, json.JSONDecodeError):
        _fail("FR0017_ATTESTATION_BUNDLE_JSON")
    if (
        ...
        _fail("FR0017_ATTESTATION_BUNDLE_SCHEMA")
    ...
    if (
        ...
        _fail("FR0017_ATTESTATION_ENVELOPE")
    try:
        ...
        _fail("FR0017_ATTESTATION_STATEMENT")
...
def validate_attestation_evidence(
    ...
    *,
    ...
    if (
        ...
        or run_id < 1
        or type(attempt) is not int
        or not 1 <= attempt <= MAX_EPOCH18_RUN_ATTEMPT
    ):
        _fail("FR0017_ATTESTATION_IDENTITY")
    ...
    if canonical_json_bytes(parsed["statement"]) != canonical_json_bytes(
        ...
        _fail("FR0017_ATTESTATION_STATEMENT_MISMATCH")
    if (
        ...
        _fail("FR0017_ATTESTATION_RECEIPT")
    ...
    if (
        ...
        _fail("FR0017_ATTESTATION_RECEIPT_BUNDLE")
    ...
    if (
        ...
        _fail("FR0017_ATTESTATION_VERIFICATION_RESULT")
    ...
    if (
        ...
        _fail("FR0017_ATTESTATION_TRANSPARENCY_LOG")
    ...
    if (
        ...
        _fail("FR0017_ATTESTATION_TRANSPARENCY_LOG_TIME")
    ...
    if not isinstance(certificate, dict) or any(
        ...
        _fail("FR0017_ATTESTATION_CERTIFICATE")
    return {
