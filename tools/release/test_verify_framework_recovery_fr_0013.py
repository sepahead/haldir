#!/usr/bin/env python3
"""Offline adversarial tests for the FR-0013 epoch-14 trust root."""
...
PROTOCOL = load_module(
    ...
    "_haldir_fr0013_test_protocol",
)
RESULT = load_module(
    "tools/release/framework_recovery_fr_0013_result.py",
    "_haldir_fr0013_test_result",
)
BRIDGE = load_module(
    "tools/release/verify-framework-recovery-fr-0013.py",
    "_haldir_fr0013_test_bridge",
)
CAPTURE = load_module(
    "tools/release/framework_recovery_fr_0013_capture.py",
    "_haldir_fr0013_test_capture",
)
PINS = load_module("tools/verify-ci-pins.py", "_haldir_fr0013_test_pins")
...
def run_documents(workflow: str = "ci") -> tuple[dict, dict]:
    if workflow == "ci":
        names = sorted(PROTOCOL.EPOCH14_CI_JOB_NAMES)
        producer = "supply-chain"
        attester = "attest-ci-audit-result"
    else:
        names = sorted(PROTOCOL.EPOCH14_FORMAL_JOB_NAMES)
...
def ruleset_documents() -> tuple[list, dict, list]:
    ruleset_id = 8_181
    summary = {
        "id": ruleset_id,
...
def branch_protection_record(qualification_commit: str = COMMIT) -> dict:
    ruleset_list, ruleset_by_id, effective_rules = ruleset_documents()
    reference = {
        "ref": "refs/heads/main",
...
class RunDocumentTests(unittest.TestCase):
    def validate(
        ...
        *,
        ...
        return PROTOCOL.validate_epoch14_run_documents(
            ordinary,
            attempt,
            workflow=workflow,
            subject_commit=COMMIT,
    ...
    def test_attempt_two_is_valid_and_fully_bound(self) -> None:
        ...
        self.assertEqual(set(result["jobs"]), PROTOCOL.EPOCH14_CI_JOB_NAMES)
        ...
        for name in ("supply-chain", "attest-ci-audit-result"):
            ...
            with (
                ...
                self.assertRaisesRegex(
                    ValueError,
                    "FR0013_EPOCH14_CURRENT_ATTEMPT_JOBS",
                ),
            ):
                self.validate(ordinary, attempt)
    ...
    def test_attempt_one_cannot_start_before_original_run(self) -> None:
        ...
        with self.assertRaisesRegex(ValueError, "EPOCH14_RUN_CHRONOLOGY"):
            self.validate(ordinary, attempt)
    ...
    def test_attempt_one_job_cannot_start_before_original_run(self) -> None:
        ...
        with self.assertRaisesRegex(ValueError, "FR0013_JOB_TIME"):
            self.validate(ordinary, attempt)
    ...
    def test_noncritical_job_outer_bounds_are_exact(self) -> None:
        ...
        for label, started_at, completed_at in attacks:
            ...
            with (
                ...
                self.assertRaisesRegex(ValueError, "FR0013_JOB_TIME"),
            ):
                self.validate(ordinary, attempt)
    ...
    def test_cross_run_job_url_is_rejected(self) -> None:
        ...
        with self.assertRaisesRegex(ValueError, "FR0013_JOB"):
            PROTOCOL.validate_epoch14_run_documents(
                ordinary,
                attempt,
                workflow="ci",
                subject_commit=COMMIT,
    ...
    def test_cross_attempt_documents_are_rejected(self) -> None:
        ordinary, attempt = run_documents()
        ordinary["attempt"] = 1
        with self.assertRaisesRegex(ValueError, "EPOCH14_RUN_METADATA"):
            PROTOCOL.validate_epoch14_run_documents(
                ordinary,
                attempt,
                workflow="ci",
    ...
    def test_ordinary_attempt_job_mismatch_is_rejected(self) -> None:
        ...
        with self.assertRaisesRegex(ValueError, "EPOCH14_RUN_JOB_MISMATCH"):
            self.validate(ordinary, attempt)
            ...
            for invalid in (True, float(valid)):
                for side in ("ordinary", "attempt"):
                    ...
                    with (
                        ...
                        self.assertRaisesRegex(ValueError, "FR0013_INTEGER"),
                    ):
                        self.validate(ordinary, attempt)
    ...
    def test_step_one_second_tolerance_is_exact(self) -> None:
        ...
        with self.assertRaisesRegex(ValueError, "FR0013_STEP_TIME"):
            self.validate(ordinary, attempt)
    ...
    def test_attempt_bound_is_fail_closed(self) -> None:
        ...
        with self.assertRaisesRegex(ValueError, "epoch14.attempt_number"):
            PROTOCOL.validate_epoch14_run_documents(
                ordinary,
                attempt,
                workflow="ci",
        ...
        for label, path, valid in mutations:
            for invalid in (True, float(valid)):
                ...
                with (
                    ...
                    self.assertRaisesRegex(ValueError, "FR0013_"),
                ):
...
class TrustedRootAndCommandTests(unittest.TestCase):
    def test_signed_fr0012_boundary_is_verified_directly(self) -> None:
        BRIDGE._verify_legacy_boundary(ROOT)

    def test_real_openssh_detached_signature_is_accepted(self) -> None:
        payload = b"FR-0013 detached signature parser integration\n"
        namespace = "haldir-fr0013-openssh-integration"
        ...
        with tempfile.TemporaryDirectory() as name:
            ...
            for mode in (0o755, 0o666):
                with self.subTest(mode=oct(mode)):
                    ...
                    with self.assertRaisesRegex(RuntimeError, "FR0013_WORKTREE"):
                        BRIDGE._verify_worktree(repo, commit, ["protected.txt"])


class RulesetTests(unittest.TestCase):
    def fixture(self) -> tuple[list, dict, list]:
        return ruleset_documents()
    ...
    def test_ruleset_integer_fields_reject_bool_and_float(self) -> None:
        mutations = (
            (
                "summary_id",
                lambda summary, detail, effective, value: summary[0].__setitem__(
                    "id", value
                ),
                8_181,
            ),
            (
                "detail_id",
                lambda summary, detail, effective, value: detail.__setitem__(
                    "id", value
                ),
                8_181,
            ),
            (
                "actor_id",
                lambda summary, detail, effective, value: detail["bypass_actors"][
                    0
            ...
            (
                "effective_id",
                lambda summary, detail, effective, value: effective[0].__setitem__(
                    "ruleset_id",
                    value,
                ),
                8_181,
        ...
        for label, mutate, valid in mutations:
            for invalid in (True, float(valid)):
                summary, detail, effective = self.fixture()
                mutate(summary, detail, effective, invalid)
                with (
                    self.subTest(label=label, invalid=invalid),
    ...
    def test_ruleset_false_parameters_reject_integer_zero(self) -> None:
        summary, detail, effective = self.fixture()
        detail["rules"][0]["parameters"]["update_allows_fetch_and_merge"] = 0
        with self.assertRaisesRegex(RuntimeError, "RULESET_RULES"):
            BRIDGE.validate_main_writer_ruleset(summary, detail, effective)

        summary, detail, effective = self.fixture()
        effective[0]["parameters"]["update_allows_fetch_and_merge"] = 0
        with self.assertRaisesRegex(RuntimeError, "RULESET_EFFECTIVE"):
            BRIDGE.validate_main_writer_ruleset(summary, detail, effective)
...
class BranchProtectionRecordTests(unittest.TestCase):
    ...
    def test_branch_repository_and_check_ids_reject_coercion(self) -> None:
        for invalid in (True, 1_292_802_592.0):
            ...
            value["repository_id"] = invalid
            with (
                self.subTest(field="repository_id", invalid=invalid),
...
class WorkflowAndPinTests(unittest.TestCase):
    ...
    def test_attempt_qualified_unarchived_outputs(self) -> None:
        for workflow in ("ci", "formal"):
            ...
            self.assertIn(
                f"epoch-14-{workflow}-result-attempt-${{{{ github.run_attempt }}}}.json",
                text,
            )
            self.assertNotIn(f"name: epoch-14-{workflow}-result-attempt", text)
    ...
    def test_gate_does_not_execute_legacy_verifier(self) -> None:
        ...
        self.assertIn("verify-framework-recovery-fr-0013.py", text)

    def test_new_root_never_imports_fr0010_fr0011_or_fr0012_python(self) -> None:
        legacy_modules = (
...
class CapturePrimitiveTests(unittest.TestCase):
    ...
    def test_epoch14_field_contracts(self) -> None:
        self.assertEqual(
            CAPTURE._run_fields(epoch14=True, attempt=False),
            (
        ...
        self.assertIn(
            "startedAt",
            CAPTURE._run_fields(epoch14=True, attempt=True),
        )
