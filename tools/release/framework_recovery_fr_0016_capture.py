#!/usr/bin/env python3
"""Capture bounded FR-0016 evidence using exact GitHub identities.
...
PROTOCOL_PATH = "tools/release/framework_recovery_fr_0016.py"
...
def _git(repo: Path, *arguments: str, limit: int = MAX_JSON_BYTES) -> bytes:
    ...
    if completed.returncode != 0 or completed.stderr or len(completed.stdout) > limit:
        _fail("FR0016_CAPTURE_GIT:" + (arguments[0] if arguments else "missing"))
...
def _repository() -> Path:
    ...
    try:
        ...
        _fail("FR0016_CAPTURE_REPOSITORY")
...
def _stage_paths(bridge: ModuleType, stage: str, workflow: str) -> tuple[str, str, str]:
    ...
    _fail("FR0016_CAPTURE_STAGE")


def _run_fields(*, epoch17: bool, attempt: bool) -> str:
    fields = [
        "attempt",
        "conclusion",
        "createdAt",
...
def _capture_hosted(
    ...
    if any(path.exists() or path.is_symlink() for path in output_paths):
        _fail("FR0016_CAPTURE_OUTPUT_EXISTS")
    if arguments.run_id < 1:
        _fail("FR0016_CAPTURE_RUN_ID")
    ...
    ordinary_fields = _run_fields(epoch17=True, attempt=False)
    ...
    if (
        ...
        or not 1 <= attempt_number <= protocol.MAX_EPOCH17_RUN_ATTEMPT
    ):
        _fail("FR0016_CAPTURE_ATTEMPT")
    attempt_fields = _run_fields(epoch17=True, attempt=True)
    attempt = _json_bytes(
        _gh(
            bridge,
            executable,
    ...
    artifact_name = f"epoch-17-{workflow}-result-attempt-{attempt_number}.json"
    listing_endpoint = (
    ...
    if type(artifact_id) is not int or artifact_id < 1:
        _fail("FR0016_CAPTURE_ARTIFACT_ID")
    ...
    if artifact_by_id != artifact:
        _fail("FR0016_CAPTURE_ARTIFACT_IDENTITY")
    result_payload = _gh(
        bridge,
        executable,
        (
    ...
    if not result_payload.endswith(b"\n"):
        _fail("FR0016_CAPTURE_DIRECT_ARTIFACT")
    materials = [
    ...
    with tempfile.TemporaryDirectory(prefix="haldir-fr0016-download-") as name:
        root = Path(name)
        ...
        if len(bundles) != 1:
            _fail("FR0016_CAPTURE_ATTESTATION_FILE")
        bundle_payload = _read_regular_bounded(
            bundles[0],
            limit=MAX_BUNDLE_BYTES,
            label="FR0016_CAPTURE_ATTESTATION_BOUND",
        )
    ...
    capture = {
        ...
        "protocol": "HALDIR_FR_0016_HOSTED_RESULT_CAPTURE_V1",
        "workflow": workflow,
        "subject_commit": subject_commit,
        "subject_tree": subject_tree,
        "expected_ref": "refs/heads/main",
    ...
    print(
        "framework-recovery-fr-0016-capture: OK "
...
def _normalize_repository_document(
    ...
    if (
        ...
        _fail("FR0016_CAPTURE_REPOSITORY_IDENTITY")
    identity = {
        "id": document["id"],
...
def _normalize_pull_request_document(
    ...
    *,
    ...
    if required_state not in {"open", "closed"}:
        _fail("FR0016_CAPTURE_PULL_REQUEST")
    ...
    if (
        ...
        or base.get("sha") != "ec207cbdba36f15a410206fd01e76a54ffd29ac0"
...
def _normalize_synthetic_merge(
    ...
    *,
    ...
    expected_parents = (
        "ec207cbdba36f15a410206fd01e76a54ffd29ac0",
        repair_commit,
    )
    if (
        not isinstance(document, dict)
        ...
        _fail("FR0016_CAPTURE_PULL_REQUEST_MERGE")
    normalized_parents = []
...
def _normalize_run_pull_request_association(
    ...
    *,
    ...
    if (
        ...
        _fail("FR0016_CAPTURE_PULL_REQUEST_RUN_ASSOCIATION")
    return {
        "run_id": run_id,
...
def _validate_open_pull_request_chronology(
    ...
    try:
        ...
        _fail("FR0016_CAPTURE_PULL_REQUEST_SNAPSHOT_CHRONOLOGY")
...
def _require_repair_head(
    ...
    try:
        ...
        _fail("FR0016_CAPTURE_PULL_REQUEST_HEAD")
    if (
...
def _external_snapshot_path(repo: Path, candidate: Path) -> Path:
    if not candidate.is_absolute() or not candidate.name:
        _fail("FR0016_CAPTURE_PULL_REQUEST_SNAPSHOT_PATH")
    try:
...
def _capture_open_pull_request(
    ...
    run_fields = _run_fields(epoch17=True, attempt=False)
    ...
    for workflow, run_id in (
        ...
        try:
            ...
            _fail("FR0016_CAPTURE_PULL_REQUEST_RUN")
        runs[workflow] = raw
    ...
    snapshot = {
        "schema_version": "1.0.0",
        "protocol": "HALDIR_FR_0016_OPEN_PULL_REQUEST_SNAPSHOT_V1",
...
def _validate_open_pull_request_snapshot(
    ...
    snapshot_payload = _read_regular_bounded(
        snapshot_path,
        limit=MAX_JSON_BYTES,
        label="FR0016_CAPTURE_PULL_REQUEST_SNAPSHOT",
    )
    ...
    try:
        ...
        _fail("FR0016_CAPTURE_PULL_REQUEST_SNAPSHOT")
    if (
        snapshot_payload != canonical_snapshot
        or set(snapshot) != expected_fields
        or snapshot["schema_version"] != "1.0.0"
        or snapshot["protocol"] != "HALDIR_FR_0016_OPEN_PULL_REQUEST_SNAPSHOT_V1"
    ):
        _fail("FR0016_CAPTURE_PULL_REQUEST_SNAPSHOT")
    ...
    if (
        ...
        != {
            "ref": "main",
            "sha": "ec207cbdba36f15a410206fd01e76a54ffd29ac0",
...
def _capture_pull_request(
    ...
    if (
        ...
        _fail("FR0016_CAPTURE_PULL_REQUEST_PHASE_DRIFT")
    run_fields = _run_fields(epoch17=True, attempt=False)
    for workflow, run_id in (
        ("ci", arguments.ci_run_id),
        ("formal", arguments.formal_run_id),
    ...
    value = {
        "schema_version": "1.0.0",
        "protocol": "HALDIR_FR_0016_PULL_REQUEST_QUALIFICATION_V1",
        "repository": repository_identity,
        "pull_request": pull_request,
        "synthetic_merge": synthetic_merge,
        "runs": snapshot["runs"],
...
def _capture_hosted_settings(
    ...
    if (
        ...
        _fail("FR0016_CAPTURE_SETTINGS_HEAD")
    ...
    vulnerability_alerts_status = _http_no_content_status(
        ...
        label="FR0016_CAPTURE_VULNERABILITY_ALERTS",
    )
    settings = {
    ...
    if (
        ...
        _fail("FR0016_CAPTURE_HOSTED_SETTINGS_POLICY")
    ...
    value = {
        "schema_version": "1.0.0",
        "protocol": "HALDIR_FR_0016_HOSTED_SETTINGS_CAPTURE_V1",
        "repository": repository_identity,
...
def _capture_branch_protection(
    ...
    if observed != arguments.qualification_commit:
        _fail("FR0016_CAPTURE_PROTECTION_HEAD")
    repository_document = _json_bytes(
        _gh(
            bridge,
            executable,
    ...
    if (
        ...
        _fail("FR0016_CAPTURE_RULESET_HISTORY")
    ...
    try:
        ...
        _fail("FR0016_CAPTURE_BRANCH_CONTROL_VALIDATION")
    ...
    value = {
        "schema_version": "1.0.0",
        "protocol": "HALDIR_FR_0016_BRANCH_PROTECTION_CAPTURE_V1",
        "repository": repository_identity,
        "branch": "main",
...
def _assemble_signed_record(
    ...
    unsigned = _read_regular_bounded(
        arguments.unsigned,
        limit=MAX_JSON_BYTES,
        label="FR0016_CAPTURE_UNSIGNED_RECORD",
    )
    signature_payload = _read_regular_bounded(
        arguments.signature,
        limit=16 * 1024,
        label="FR0016_CAPTURE_RECORD_SIGNATURE",
    )
    try:
        ...
        _fail("FR0016_CAPTURE_UNSIGNED_RECORD")
...
def _resolve_local_cargo(
    ...
    if candidate is None:
        _fail("FR0016_CAPTURE_LOCAL_RUSTUP")
    rustup, rustup_payload = _validated_local_executable(
        repo,
        Path(candidate),
        label="FR0016_CAPTURE_LOCAL_RUSTUP",
    )
    ...
    if returncode != 0 or stderr or stdout.count(b"\n") != 1:
        _fail("FR0016_CAPTURE_LOCAL_CARGO")
    try:
    ...
    cargo, cargo_payload = _validated_local_executable(
        repo,
        cargo_candidate,
        label="FR0016_CAPTURE_LOCAL_CARGO",
    )
    cargo_environment = {
        "LC_ALL": "C",
...
def _capture_local(
    ...
    if (
        ...
        _fail("FR0016_CAPTURE_LOCAL_HEAD")
    _require_clean_worktree(
        repo,
        label="FR0016_CAPTURE_LOCAL_WORKTREE",
    )
    ...
    if "HALDIR_FR0016_GH" in os.environ:
        environment["HALDIR_FR0016_GH"] = os.environ["HALDIR_FR0016_GH"]
    ...
    for command in commands:
        ...
        if type(returncode) is not int or returncode != 0:
            _fail("FR0016_CAPTURE_LOCAL_COMMAND")
        checks.append(
            {
                "argv": list(command),
                "returncode": 0,
    ...
    value = {
        "schema_version": "1.0.0",
        "protocol": "HALDIR_FR_0016_LOCAL_VALIDATION_V1",
        "subject_commit": repair_commit,
    ...
    _write_exclusive(output, bridge.canonical_json_bytes(value))
    print("framework-recovery-fr-0016-capture: OK (local R validation; release NO_GO)")
...
def _verify_root(
    ...
    for line in raw.splitlines(keepends=True):
        try:
            ...
            _fail("FR0016_CAPTURE_TRUSTED_ROOT")
    ...
    if matches != [expected]:
        _fail("FR0016_CAPTURE_TRUSTED_ROOT_DRIFT")
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        repo = _repository()
        bridge = _load_module(repo / BRIDGE_PATH, "_haldir_fr0016_capture_bridge")
        protocol = _load_module(repo / PROTOCOL_PATH, "_haldir_fr0016_capture_protocol")
        ...
        print(f"framework-recovery-fr-0016-capture: {error}", file=sys.stderr)
        return 1
    return 0
