#!/usr/bin/env python3
"""Capture bounded FR-0010 evidence using exact GitHub identities.

This tool acquires bytes; it does not grant authority.  The central bridge
replays every parser and independently performs offline Sigstore verification
against the trust root frozen by the signed R commit.
"""

from __future__ import annotations

import argparse
import hashlib
import importlib.util
import json
import os
import re
import subprocess
import sys
import tempfile
from datetime import datetime, timezone
from pathlib import Path
from types import ModuleType
from typing import Any, Sequence


REPOSITORY = "sepahead/haldir"
HEX40 = re.compile(r"^[0-9a-f]{40}$")
MAX_JSON_BYTES = 2 * 1024 * 1024
MAX_ARCHIVE_BYTES = 2 * 1024 * 1024
MAX_BUNDLE_BYTES = 1024 * 1024
MAX_PROVIDER_STDERR = 64 * 1024
BRIDGE_PATH = "tools/release/verify-framework-recovery-fr-0010.py"
PROTOCOL_PATH = "tools/release/framework_recovery_fr_0010.py"


class CaptureError(RuntimeError):
    """One fail-closed capture error."""


def _fail(code: str) -> None:
    raise CaptureError(code)


def _load_module(path: Path, name: str) -> ModuleType:
    specification = importlib.util.spec_from_file_location(name, path)
    if specification is None or specification.loader is None:
        _fail("FR0010_CAPTURE_MODULE")
    module = importlib.util.module_from_spec(specification)
    specification.loader.exec_module(module)
    return module


def _git_environment() -> dict[str, str]:
    return {
        "GIT_CONFIG_GLOBAL": os.devnull,
        "GIT_CONFIG_NOSYSTEM": "1",
        "GIT_NO_REPLACE_OBJECTS": "1",
        "LC_ALL": "C",
        "PATH": "/usr/bin:/bin",
    }


def _git(repo: Path, *arguments: str, limit: int = MAX_JSON_BYTES) -> bytes:
    completed = subprocess.run(
        ("/usr/bin/git", "-c", "core.hooksPath=/dev/null", *arguments),
        cwd=repo,
        env=_git_environment(),
        stdin=subprocess.DEVNULL,
        stdout=subprocess.PIPE,
        stderr=subprocess.PIPE,
        check=False,
        timeout=30,
    )
    if (
        completed.returncode != 0
        or completed.stderr
        or len(completed.stdout) > limit
    ):
        _fail("FR0010_CAPTURE_GIT:" + (arguments[0] if arguments else "missing"))
    return completed.stdout


def _repository() -> Path:
    raw = _git(Path.cwd(), "rev-parse", "--show-toplevel", limit=64 * 1024)
    try:
        return Path(raw.decode("utf-8").strip()).resolve(strict=True)
    except (OSError, UnicodeDecodeError):
        _fail("FR0010_CAPTURE_REPOSITORY")


def _online_environment(executable: Path) -> dict[str, str]:
    environment = {
        "LC_ALL": "C",
        "PATH": f"{executable.parent}:/usr/bin:/bin",
    }
    for name in (
        "GH_CONFIG_DIR",
        "GH_ENTERPRISE_TOKEN",
        "GH_HOST",
        "GH_TOKEN",
        "GITHUB_ENTERPRISE_TOKEN",
        "GITHUB_TOKEN",
        "HOME",
        "HTTPS_PROXY",
        "HTTP_PROXY",
        "NO_PROXY",
        "SSL_CERT_FILE",
        "XDG_CONFIG_HOME",
    ):
        if name in os.environ:
            environment[name] = os.environ[name]
    return environment


def _gh(
    bridge: ModuleType,
    executable: Path,
    arguments: tuple[str, ...],
    *,
    cwd: Path,
    limit: int,
    timeout: float = 60,
) -> bytes:
    returncode, stdout, stderr = bridge._run_bounded(
        (str(executable), *arguments),
        cwd=cwd,
        env=_online_environment(executable),
        timeout_seconds=timeout,
        output_limit=limit + MAX_PROVIDER_STDERR,
    )
    if (
        returncode != 0
        or len(stdout) > limit
        or len(stderr) > MAX_PROVIDER_STDERR
    ):
        _fail("FR0010_CAPTURE_GH:" + (arguments[0] if arguments else "missing"))
    return stdout


def _json_bytes(raw: bytes, label: str) -> dict[str, Any]:
    if not raw or len(raw) > MAX_JSON_BYTES or b"\0" in raw:
        _fail("FR0010_CAPTURE_JSON:" + label)
    try:
        value = json.loads(raw)
    except (UnicodeDecodeError, json.JSONDecodeError):
        _fail("FR0010_CAPTURE_JSON:" + label)
    if not isinstance(value, dict):
        _fail("FR0010_CAPTURE_JSON:" + label)
    return value


def _json_list(raw: bytes, label: str) -> list[Any]:
    if not raw or len(raw) > MAX_JSON_BYTES or b"\0" in raw:
        _fail("FR0010_CAPTURE_JSON:" + label)
    try:
        value = json.loads(raw)
    except (UnicodeDecodeError, json.JSONDecodeError):
        _fail("FR0010_CAPTURE_JSON:" + label)
    if not isinstance(value, list):
        _fail("FR0010_CAPTURE_JSON:" + label)
    return value


def _now() -> str:
    return (
        datetime.now(timezone.utc)
        .isoformat(timespec="microseconds")
        .replace("+00:00", "Z")
    )


def _logical_path(repo: Path, path: Path) -> str:
    candidate = path if path.is_absolute() else repo / path
    resolved_parent = candidate.parent.resolve(strict=True)
    resolved = resolved_parent / candidate.name
    try:
        relative = resolved.relative_to(repo)
    except ValueError:
        _fail("FR0010_CAPTURE_OUTPUT_SCOPE")
    value = relative.as_posix()
    if (
        value.startswith("/")
        or "//" in value
        or any(part in {"", ".", ".."} for part in value.split("/"))
    ):
        _fail("FR0010_CAPTURE_OUTPUT_SCOPE")
    return value


def _write_exclusive(path: Path, payload: bytes, *, mode: int = 0o644) -> None:
    descriptor = os.open(
        path,
        os.O_WRONLY | os.O_CREAT | os.O_EXCL | getattr(os, "O_NOFOLLOW", 0),
        mode,
    )
    try:
        view = memoryview(payload)
        while view:
            written = os.write(descriptor, view)
            if written < 1:
                _fail("FR0010_CAPTURE_WRITE")
            view = view[written:]
        os.fsync(descriptor)
    finally:
        os.close(descriptor)


def _blob_record(path: str, payload: bytes, *, mode: str = "100644") -> dict[str, Any]:
    framed = b"blob " + str(len(payload)).encode("ascii") + b"\0" + payload
    return {
        "path": path,
        "git_mode": mode,
        "git_object_type": "blob",
        "git_object_id": hashlib.sha1(framed, usedforsecurity=False).hexdigest(),
        "sha256": hashlib.sha256(payload).hexdigest(),
        "bytes": len(payload),
    }


def _commit_tree(repo: Path, commit: str) -> str:
    if HEX40.fullmatch(commit) is None:
        _fail("FR0010_CAPTURE_COMMIT")
    value = _git(repo, "rev-parse", "--verify", f"{commit}^{{tree}}").decode().strip()
    if HEX40.fullmatch(value) is None:
        _fail("FR0010_CAPTURE_COMMIT")
    return value


def _stage_paths(bridge: ModuleType, stage: str, workflow: str) -> tuple[str, str, str]:
    if stage == "repair":
        return (
            bridge.REPAIR_CI_PATHS
            if workflow == "ci"
            else bridge.REPAIR_FORMAL_PATHS
        )
    if stage == "qualification":
        return (
            bridge.QUALIFICATION_CI_PATHS
            if workflow == "ci"
            else bridge.QUALIFICATION_FORMAL_PATHS
        )
    _fail("FR0010_CAPTURE_STAGE")


def _run_fields(*, epoch11: bool, attempt: bool) -> str:
    fields = [
        "attempt",
        "conclusion",
        "createdAt",
        "databaseId",
        "event",
        "headBranch",
        "headSha",
        "jobs",
    ]
    if epoch11:
        fields.append("number")
    if attempt:
        fields.append("startedAt")
    fields.extend(("status", "updatedAt", "url"))
    if attempt or epoch11:
        fields.append("workflowDatabaseId")
    fields.append("workflowName")
    return ",".join(fields)


def _capture_hosted(
    repo: Path,
    bridge: ModuleType,
    protocol: ModuleType,
    arguments: argparse.Namespace,
) -> None:
    workflow = arguments.workflow
    expected_paths = _stage_paths(bridge, arguments.stage, workflow)
    output_paths = tuple(repo / path for path in expected_paths)
    if any(path.exists() or path.is_symlink() for path in output_paths):
        _fail("FR0010_CAPTURE_OUTPUT_EXISTS")
    if arguments.run_id < 1:
        _fail("FR0010_CAPTURE_RUN_ID")
    repair_commit = arguments.repair_commit
    subject_commit = arguments.subject_commit
    _commit_tree(repo, repair_commit)
    subject_tree = _commit_tree(repo, subject_commit)
    executable, version = bridge._trusted_gh()
    ordinary_fields = _run_fields(epoch11=True, attempt=False)
    ordinary = _json_bytes(
        _gh(
            bridge,
            executable,
            (
                "run",
                "view",
                str(arguments.run_id),
                "--repo",
                REPOSITORY,
                "--json",
                ordinary_fields,
            ),
            cwd=repo,
            limit=MAX_JSON_BYTES,
        ),
        "ordinary",
    )
    attempt_number = ordinary.get("attempt")
    if (
        type(attempt_number) is not int
        or not 1 <= attempt_number <= protocol.MAX_EPOCH11_RUN_ATTEMPT
    ):
        _fail("FR0010_CAPTURE_ATTEMPT")
    attempt_fields = _run_fields(epoch11=True, attempt=True)
    attempt = _json_bytes(
        _gh(
            bridge,
            executable,
            (
                "run",
                "view",
                str(arguments.run_id),
                "--repo",
                REPOSITORY,
                "--attempt",
                str(attempt_number),
                "--json",
                attempt_fields,
            ),
            cwd=repo,
            limit=MAX_JSON_BYTES,
        ),
        "attempt",
    )
    run = protocol.validate_epoch11_run_documents(
        ordinary,
        attempt,
        workflow=workflow,
        subject_commit=subject_commit,
        expected_ref="refs/heads/main",
    )
    artifact_name = (
        f"epoch-11-{workflow}-result-attempt-{attempt_number}.json"
    )
    listing_endpoint = (
        f"repos/{REPOSITORY}/actions/runs/{arguments.run_id}/artifacts"
        f"?name={artifact_name}&per_page=100"
    )
    listing = _json_bytes(
        _gh(
            bridge,
            executable,
            ("api", "--method", "GET", listing_endpoint),
            cwd=repo,
            limit=MAX_JSON_BYTES,
        ),
        "artifact-list",
    )
    if (
        set(listing) != {"artifacts", "total_count"}
        or listing["total_count"] != 1
        or not isinstance(listing["artifacts"], list)
        or len(listing["artifacts"]) != 1
        or not isinstance(listing["artifacts"][0], dict)
    ):
        _fail("FR0010_CAPTURE_ARTIFACT_UNIQUENESS")
    artifact = listing["artifacts"][0]
    artifact_id = artifact.get("id")
    if type(artifact_id) is not int or artifact_id < 1:
        _fail("FR0010_CAPTURE_ARTIFACT_ID")
    artifact_by_id = _json_bytes(
        _gh(
            bridge,
            executable,
            (
                "api",
                "--method",
                "GET",
                f"repos/{REPOSITORY}/actions/artifacts/{artifact_id}",
            ),
            cwd=repo,
            limit=MAX_JSON_BYTES,
        ),
        "artifact-id",
    )
    if artifact_by_id != artifact:
        _fail("FR0010_CAPTURE_ARTIFACT_IDENTITY")
    result_payload = _gh(
        bridge,
        executable,
        (
            "api",
            "--method",
            "GET",
            f"repos/{REPOSITORY}/actions/artifacts/{artifact_id}/zip",
        ),
        cwd=repo,
        limit=256 * 1024,
    )
    if not result_payload.endswith(b"\n"):
        _fail("FR0010_CAPTURE_DIRECT_ARTIFACT")
    materials = [
        bridge.file_record(repo, subject_commit, path)
        for path in protocol.RESULT_CONTRACT[workflow]["material_paths"]
    ]
    protocol.validate_result_artifact(
        result_payload,
        workflow=workflow,
        subject_commit=subject_commit,
        subject_tree=subject_tree,
        run_id=run["run_id"],
        attempt=run["attempt"],
        run_number=run["run_number"],
        expected_ref="refs/heads/main",
        expected_materials=materials,
    )
    protocol.validate_artifact_metadata(
        artifact,
        workflow=workflow,
        run_id=run["run_id"],
        attempt=run["attempt"],
        subject_commit=subject_commit,
        result_payload=result_payload,
        producer_started=run["jobs"][
            protocol.RESULT_CONTRACT[workflow]["job"]
        ]["started"],
        producer_completed=run["jobs"][
            protocol.RESULT_CONTRACT[workflow]["job"]
        ]["completed"],
        attestation_started=run["jobs"][run["attestation_job"]]["started"],
    )
    trusted_root_payload = (repo / bridge.TRUSTED_ROOT_PATH).read_bytes()
    bridge._validate_trusted_root(trusted_root_payload)
    with tempfile.TemporaryDirectory(prefix="haldir-fr0010-download-") as name:
        root = Path(name)
        result_file = root / artifact_name
        bridge._write_private(result_file, result_payload)
        _gh(
            bridge,
            executable,
            (
                "attestation",
                "download",
                artifact_name,
                "--repo",
                REPOSITORY,
                "--limit",
                "1",
                "--predicate-type",
                "https://slsa.dev/provenance/v1",
            ),
            cwd=root,
            limit=64 * 1024,
        )
        digest = hashlib.sha256(result_payload).hexdigest()
        candidates = (
            root / f"sha256:{digest}.jsonl",
            root / f"sha256-{digest}.jsonl",
        )
        bundles = [
            path
            for path in candidates
            if path.is_file() and not path.is_symlink()
        ]
        if len(bundles) != 1:
            _fail("FR0010_CAPTURE_ATTESTATION_FILE")
        bundle_payload = bundles[0].read_bytes()
    if not 1 <= len(bundle_payload) <= MAX_BUNDLE_BYTES:
        _fail("FR0010_CAPTURE_ATTESTATION_BOUND")
    receipt, verification = bridge._verify_attestation_offline(
        result_payload=result_payload,
        bundle_payload=bundle_payload,
        trusted_root_payload=trusted_root_payload,
        workflow=workflow,
        subject_commit=subject_commit,
        attempt=attempt_number,
    )
    protocol.validate_attestation_evidence(
        bundle_payload,
        receipt,
        workflow=workflow,
        result_payload=result_payload,
        subject_commit=subject_commit,
        expected_ref="refs/heads/main",
        run_id=run["run_id"],
        attempt=run["attempt"],
        attestation_started=run["jobs"][run["attestation_job"]]["started"],
        attestation_completed=run["jobs"][run["attestation_job"]]["completed"],
    )
    result_path, bundle_path = expected_paths[1], expected_paths[2]
    capture = {
        "schema_version": "1.0.0",
        "protocol": "HALDIR_FR_0010_HOSTED_RESULT_CAPTURE_V1",
        "workflow": workflow,
        "subject_commit": subject_commit,
        "subject_tree": subject_tree,
        "expected_ref": "refs/heads/main",
        "ordinary": ordinary,
        "attempt_metadata": attempt,
        "artifact_listing": listing,
        "artifact_by_id": artifact_by_id,
        "artifact": artifact,
        "artifact_download": {
            "bytes": len(result_payload),
            "content_mode": "DIRECT_UNARCHIVED_FILE",
            "sha256": hashlib.sha256(result_payload).hexdigest(),
        },
        "commands": bridge._hosted_commands(
            workflow=workflow,
            run_id=run["run_id"],
            attempt=run["attempt"],
            artifact_id=artifact_id,
        ),
        "capture_tool": bridge.file_record(
            repo, repair_commit, bridge.CAPTURE_PATH
        ),
        "result_record": _blob_record(result_path, result_payload),
        "attestation_record": _blob_record(bundle_path, bundle_payload),
        "trusted_root_record": bridge.file_record(
            repo, repair_commit, bridge.TRUSTED_ROOT_PATH
        ),
        "attestation_verification": receipt,
        "capture_verification": verification,
        "captured_at_utc": _now(),
        "result": "PASS",
    }
    capture_payload = bridge.canonical_json_bytes(capture)
    _write_exclusive(output_paths[1], result_payload)
    _write_exclusive(output_paths[2], bundle_payload)
    _write_exclusive(output_paths[0], capture_payload)
    print(
        "framework-recovery-fr-0010-capture: OK "
        f"({arguments.stage}; {workflow}; run {run['run_id']}; "
        f"attempt {run['attempt']}; artifact {artifact_id})"
    )


def _capture_c5(
    repo: Path,
    bridge: ModuleType,
    protocol: ModuleType,
    arguments: argparse.Namespace,
) -> None:
    workflow = arguments.workflow
    paths = bridge.C5_CI_PATHS if workflow == "ci" else bridge.C5_FORMAL_PATHS
    outputs = tuple(repo / path for path in paths)
    if any(path.exists() or path.is_symlink() for path in outputs):
        _fail("FR0010_CAPTURE_OUTPUT_EXISTS")
    executable, _version = bridge._trusted_gh()
    started = _now()
    ordinary_fields = _run_fields(epoch11=False, attempt=False)
    ordinary = _json_bytes(
        _gh(
            bridge,
            executable,
            (
                "run",
                "view",
                str(arguments.run_id),
                "--repo",
                REPOSITORY,
                "--json",
                ordinary_fields,
            ),
            cwd=repo,
            limit=MAX_JSON_BYTES,
        ),
        "c5-ordinary",
    )
    attempt_number = ordinary.get("attempt")
    if type(attempt_number) is not int or attempt_number < 1:
        _fail("FR0010_CAPTURE_ATTEMPT")
    attempt_fields = _run_fields(epoch11=False, attempt=True)
    attempt = _json_bytes(
        _gh(
            bridge,
            executable,
            (
                "run",
                "view",
                str(arguments.run_id),
                "--repo",
                REPOSITORY,
                "--attempt",
                str(attempt_number),
                "--json",
                attempt_fields,
            ),
            cwd=repo,
            limit=MAX_JSON_BYTES,
        ),
        "c5-attempt",
    )
    archive = _gh(
        bridge,
        executable,
        (
            "api",
            "--method",
            "GET",
            f"repos/{REPOSITORY}/actions/runs/{arguments.run_id}/attempts/"
            f"{attempt_number}/logs",
        ),
        cwd=repo,
        limit=MAX_ARCHIVE_BYTES,
    )
    run = protocol.validate_c5_run_documents(
        ordinary,
        attempt,
        workflow=workflow,
        subject_commit=bridge.PARENT,
    )
    suite_counts = [163, 78, 94, 26, 30, 44, 56, 37, 55, 60]
    archive_result = (
        protocol.validate_c5_ci_archive(
            archive, jobs=run["jobs"], suite_counts=suite_counts
        )
        if workflow == "ci"
        else protocol.validate_c5_formal_archive(archive, jobs=run["jobs"])
    )
    fields = _run_fields(epoch11=False, attempt=False)
    attempt_fields = _run_fields(epoch11=False, attempt=True)
    completed = _now()
    value = {
        "schema_version": "1.0.0",
        "protocol": protocol.C5_ARCHIVE_PROTOCOL,
        "workflow": workflow,
        "subject_commit": bridge.PARENT,
        "ordinary": ordinary,
        "attempt_metadata": attempt,
        "archive": archive_result,
        "capture": {
            "tool": bridge.file_record(
                repo, arguments.repair_commit, bridge.CAPTURE_PATH
            ),
            "metadata_command": (
                f"gh run view {arguments.run_id} --repo {REPOSITORY} "
                f"--json {fields}"
            ),
            "attempt_command": (
                f"gh run view {arguments.run_id} --repo {REPOSITORY} "
                f"--attempt {attempt_number} --json {attempt_fields}"
            ),
            "archive_command": (
                "gh api --method GET "
                f"repos/{REPOSITORY}/actions/runs/{arguments.run_id}/attempts/"
                f"{attempt_number}/logs"
            ),
            "started_at_utc": started,
            "completed_at_utc": completed,
        },
        "result": "PASS",
    }
    protocol.validate_c5_hosted_capture(
        value,
        archive,
        workflow=workflow,
        subject_commit=bridge.PARENT,
        suite_counts=suite_counts if workflow == "ci" else None,
    )
    _write_exclusive(outputs[1], archive)
    _write_exclusive(outputs[0], bridge.canonical_json_bytes(value))
    print(
        "framework-recovery-fr-0010-capture: OK "
        f"(c5 {workflow}; run {run['run_id']}; diagnostic only)"
    )


def _capture_branch_protection(
    repo: Path,
    bridge: ModuleType,
    arguments: argparse.Namespace,
) -> None:
    output = repo / bridge.BRANCH_PROTECTION_PATH
    if output.exists() or output.is_symlink():
        _fail("FR0010_CAPTURE_OUTPUT_EXISTS")
    executable, _version = bridge._trusted_gh()
    reference = _json_bytes(
        _gh(
            bridge,
            executable,
            (
                "api",
                "--method",
                "GET",
                f"repos/{REPOSITORY}/git/ref/heads/main",
            ),
            cwd=repo,
            limit=MAX_JSON_BYTES,
        ),
        "main-ref",
    )
    object_value = reference.get("object")
    observed = (
        object_value.get("sha") if isinstance(object_value, dict) else None
    )
    if observed != arguments.qualification_commit:
        _fail("FR0010_CAPTURE_PROTECTION_HEAD")
    protection = _json_bytes(
        _gh(
            bridge,
            executable,
            (
                "api",
                "--method",
                "GET",
                f"repos/{REPOSITORY}/branches/main/protection",
            ),
            cwd=repo,
            limit=MAX_JSON_BYTES,
        ),
        "protection",
    )
    ruleset_list = _json_list(
        _gh(
            bridge,
            executable,
            (
                "api",
                "--method",
                "GET",
                f"repos/{REPOSITORY}/rulesets",
            ),
            cwd=repo,
            limit=MAX_JSON_BYTES,
        ),
        "ruleset-list",
    )
    ruleset_matches = [
        item
        for item in ruleset_list
        if isinstance(item, dict)
        and item.get("name") == "haldir-main-writer-allowlist"
        and item.get("target") == "branch"
        and item.get("enforcement") == "active"
    ]
    if len(ruleset_matches) != 1:
        _fail("FR0010_CAPTURE_RULESET_UNIQUENESS")
    ruleset_id = ruleset_matches[0].get("id")
    if type(ruleset_id) is not int or ruleset_id < 1:
        _fail("FR0010_CAPTURE_RULESET_ID")
    ruleset_by_id = _json_bytes(
        _gh(
            bridge,
            executable,
            (
                "api",
                "--method",
                "GET",
                f"repos/{REPOSITORY}/rulesets/{ruleset_id}",
            ),
            cwd=repo,
            limit=MAX_JSON_BYTES,
        ),
        "ruleset-id",
    )
    effective_rules = _json_list(
        _gh(
            bridge,
            executable,
            (
                "api",
                "--method",
                "GET",
                f"repos/{REPOSITORY}/rules/branches/main",
            ),
            cwd=repo,
            limit=MAX_JSON_BYTES,
        ),
        "effective-rules",
    )
    reference_after = _json_bytes(
        _gh(
            bridge,
            executable,
            (
                "api",
                "--method",
                "GET",
                f"repos/{REPOSITORY}/git/ref/heads/main",
            ),
            cwd=repo,
            limit=MAX_JSON_BYTES,
        ),
        "main-ref-after",
    )
    object_after = reference_after.get("object")
    observed_after = (
        object_after.get("sha") if isinstance(object_after, dict) else None
    )
    if observed_after != arguments.qualification_commit:
        _fail("FR0010_CAPTURE_PROTECTION_HEAD")
    value = {
        "schema_version": "1.0.0",
        "protocol": "HALDIR_FR_0010_BRANCH_PROTECTION_CAPTURE_V1",
        "repository": REPOSITORY,
        "repository_id": 1_292_802_592,
        "branch": "main",
        "observed_commit": observed,
        "ref_before": reference,
        "ref_after": reference_after,
        "protection": protection,
        "ruleset_list": ruleset_list,
        "ruleset_by_id": ruleset_by_id,
        "effective_rules": effective_rules,
        "capture": {
            "commit_before_command": (
                f"gh api --method GET repos/{REPOSITORY}/git/ref/heads/main"
            ),
            "commit_after_command": (
                f"gh api --method GET repos/{REPOSITORY}/git/ref/heads/main"
            ),
            "protection_command": (
                f"gh api --method GET repos/{REPOSITORY}/branches/main/protection"
            ),
            "ruleset_list_command": (
                f"gh api --method GET repos/{REPOSITORY}/rulesets"
            ),
            "ruleset_get_command": (
                f"gh api --method GET repos/{REPOSITORY}/rulesets/{ruleset_id}"
            ),
            "effective_rules_command": (
                f"gh api --method GET repos/{REPOSITORY}/rules/branches/main"
            ),
            "captured_at_utc": _now(),
            "transport": "GITHUB_API_OVER_TLS",
            "result": "PASS",
        },
        "authority": {
            "cryptographic_proof": False,
            "durable_external_state_proof": False,
            "release_authority": False,
            "transport_observation": "GITHUB_API_OVER_TLS",
        },
    }
    _write_exclusive(output, bridge.canonical_json_bytes(value))
    print(
        "framework-recovery-fr-0010-capture: OK "
        "(branch protection TLS observation; non-cryptographic)"
    )


def _review_contract(
    bridge: ModuleType, review_id: str
) -> tuple[str, tuple[str, str], list[dict[str, Any]]]:
    if review_id == "FR-0010-R01":
        model = "claude-fable-5"
        paths = bridge.DESIGN_REVIEW_PATHS
    elif review_id == "FR-0010-R02":
        model = "claude-opus-5"
        paths = bridge.IMPLEMENTATION_REVIEW_PATHS
    else:
        _fail("FR0010_CAPTURE_REVIEW_ID")
    findings = [
        {
            "id": "F001",
            "summary": "Normative hosted proof is artifact and OIDC based.",
            "affected_paths": [
                bridge.MODULE_PATH,
                ".github/workflows/ci.yml",
                ".github/workflows/formal.yml",
            ],
        },
        {
            "id": "F002",
            "summary": "Legacy successor recursion is retired after activation.",
            "affected_paths": [bridge.BRIDGE_PATH, bridge.PLAN_PATH],
        },
        {
            "id": "F003",
            "summary": "Exact model response bytes are retained and reparsed.",
            "affected_paths": [bridge.CAPTURE_PATH, paths[1]],
        },
    ]
    return model, paths, findings


def _review_material(
    repo: Path,
    bridge: ModuleType,
    protocol: ModuleType,
    *,
    review_id: str,
    repair_commit: str,
) -> tuple[dict[str, Any], bytes]:
    model, _paths, findings = _review_contract(bridge, review_id)
    plan_payload = bridge._file(repo, repair_commit, bridge.PLAN_PATH)
    patch_payload = bridge._core_patch(repo, repair_commit)
    gate_payload = bridge._file(repo, repair_commit, bridge.GATE_PATH)
    manifest = protocol.review_subject_manifest(
        review_id=review_id,
        model=model,
        repair_commit=repair_commit,
        plan_sha256=hashlib.sha256(plan_payload).hexdigest(),
        patch_sha256=hashlib.sha256(patch_payload).hexdigest(),
        gate_sha256=hashlib.sha256(gate_payload).hexdigest(),
        required_findings=findings,
    )
    request = protocol.build_review_request(
        manifest=manifest,
        plan_payload=plan_payload,
        patch_payload=patch_payload,
        gate_payload=gate_payload,
    )
    return manifest, request


def _review_request(
    repo: Path,
    bridge: ModuleType,
    protocol: ModuleType,
    arguments: argparse.Namespace,
) -> None:
    _manifest, request = _review_material(
        repo,
        bridge,
        protocol,
        review_id=arguments.review_id,
        repair_commit=arguments.repair_commit,
    )
    output = arguments.output
    if not output.is_absolute():
        output = Path.cwd() / output
    if output.exists() or output.is_symlink():
        _fail("FR0010_CAPTURE_OUTPUT_EXISTS")
    _write_exclusive(output, request, mode=0o600)
    print(
        "framework-recovery-fr-0010-capture: OK "
        f"({arguments.review_id} request; sha256 "
        f"{hashlib.sha256(request).hexdigest()})"
    )


def _capture_review(
    repo: Path,
    bridge: ModuleType,
    protocol: ModuleType,
    arguments: argparse.Namespace,
) -> None:
    model, paths, _findings = _review_contract(bridge, arguments.review_id)
    outputs = tuple(repo / path for path in paths)
    if any(path.exists() or path.is_symlink() for path in outputs):
        _fail("FR0010_CAPTURE_OUTPUT_EXISTS")
    manifest, request = _review_material(
        repo,
        bridge,
        protocol,
        review_id=arguments.review_id,
        repair_commit=arguments.repair_commit,
    )
    if arguments.response.is_symlink():
        _fail("FR0010_CAPTURE_REVIEW_RESPONSE")
    response_input = arguments.response.resolve(strict=True)
    if (
        not response_input.is_file()
        or response_input.stat().st_size > protocol.MAX_REVIEW_RESPONSE_BYTES
    ):
        _fail("FR0010_CAPTURE_REVIEW_RESPONSE")
    response = response_input.read_bytes()
    outcome = protocol.parse_review_response_bytes(
        response, manifest=manifest
    )
    capture = {
        "schema_version": "1.0.0",
        "protocol": "HALDIR_FR_0010_REVIEW_CAPTURE_V1",
        "review_id": arguments.review_id,
        "model": model,
        "manifest": manifest,
        "request_sha256": hashlib.sha256(request).hexdigest(),
        "request_bytes": len(request),
        "response_record": _blob_record(paths[1], response),
        "parsed_outcome": outcome,
        "provider_provenance_independently_attested": False,
        "review_authority_conferred": False,
    }
    _write_exclusive(outputs[1], response)
    _write_exclusive(outputs[0], bridge.canonical_json_bytes(capture))
    print(
        "framework-recovery-fr-0010-capture: OK "
        f"({arguments.review_id}; {outcome['verdict']}; "
        "provider provenance not independently attested)"
    )


def _capture_local(
    repo: Path,
    bridge: ModuleType,
    arguments: argparse.Namespace,
) -> None:
    output = repo / bridge.LOCAL_PATH
    if output.exists() or output.is_symlink():
        _fail("FR0010_CAPTURE_OUTPUT_EXISTS")
    repair_commit = arguments.repair_commit
    if (
        _git(repo, "rev-parse", "--verify", "HEAD^{commit}").decode().strip()
        != repair_commit
    ):
        _fail("FR0010_CAPTURE_LOCAL_HEAD")
    python = Path(sys.executable).resolve(strict=True)
    commands = (
        (
            str(python),
            "-I",
            "-B",
            "-W",
            "error",
            "tools/release/test_verify_framework_recovery_fr_0010.py",
        ),
        (
            str(python),
            "-I",
            "-B",
            "-W",
            "error",
            "tools/verify-ci-pins.py",
        ),
        (
            str(python),
            "-I",
            "-B",
            "-W",
            "error",
            "tools/release/verify-framework-recovery-fr-0010.py",
        ),
        (
            "/usr/bin/git",
            "-c",
            "core.hooksPath=/dev/null",
            "diff",
            "--check",
            f"{bridge.PARENT}..{repair_commit}",
        ),
    )
    environment = {
        "GIT_CONFIG_GLOBAL": os.devnull,
        "GIT_CONFIG_NOSYSTEM": "1",
        "GIT_NO_REPLACE_OBJECTS": "1",
        "HOME": str(Path.home()),
        "LC_ALL": "C",
        "PATH": f"{python.parent}:/usr/bin:/bin",
        "PYTHONDONTWRITEBYTECODE": "1",
        "PYTHONHASHSEED": "0",
    }
    if "HALDIR_FR0010_GH" in os.environ:
        environment["HALDIR_FR0010_GH"] = os.environ["HALDIR_FR0010_GH"]
    if "RUNNER_TEMP" in os.environ:
        environment["RUNNER_TEMP"] = os.environ["RUNNER_TEMP"]
    checks = []
    for command in commands:
        returncode, stdout, stderr = bridge._run_bounded(
            command,
            cwd=repo,
            env=environment,
            timeout_seconds=180,
            output_limit=4 * 1024 * 1024,
        )
        if returncode != 0:
            _fail("FR0010_CAPTURE_LOCAL_COMMAND")
        checks.append(
            {
                "argv": list(command),
                "returncode": 0,
                "stdout_bytes": len(stdout),
                "stdout_sha256": hashlib.sha256(stdout).hexdigest(),
                "stderr_bytes": len(stderr),
                "stderr_sha256": hashlib.sha256(stderr).hexdigest(),
                "result": "PASS",
            }
        )
    value = {
        "schema_version": "1.0.0",
        "protocol": "HALDIR_FR_0010_LOCAL_VALIDATION_V1",
        "subject_commit": repair_commit,
        "subject_tree": _commit_tree(repo, repair_commit),
        "capture_tool": bridge.file_record(
            repo, repair_commit, bridge.CAPTURE_PATH
        ),
        "python": {
            "implementation": sys.implementation.name,
            "version": (
                f"{sys.version_info.major}.{sys.version_info.minor}."
                f"{sys.version_info.micro}"
            ),
        },
        "checks": checks,
        "completed_at_utc": _now(),
        "result": "PASS",
        "authority": bridge._authority("PENDING_QUALIFICATION"),
    }
    _write_exclusive(output, bridge.canonical_json_bytes(value))
    print(
        "framework-recovery-fr-0010-capture: OK "
        "(local R validation; release NO_GO)"
    )


def _verify_root(
    repo: Path,
    bridge: ModuleType,
) -> None:
    executable, version = bridge._trusted_gh()
    raw = _gh(
        bridge,
        executable,
        ("attestation", "trusted-root"),
        cwd=repo,
        limit=bridge.MAX_TRUSTED_ROOT_BYTES,
        timeout=120,
    )
    matches: list[bytes] = []
    for line in raw.splitlines(keepends=True):
        try:
            value = json.loads(line)
        except (UnicodeDecodeError, json.JSONDecodeError):
            _fail("FR0010_CAPTURE_TRUSTED_ROOT")
        tlogs = value.get("tlogs", []) if isinstance(value, dict) else []
        if {
            item.get("baseUrl")
            for item in tlogs
            if isinstance(item, dict)
        } == {
            "https://rekor.sigstore.dev",
            "https://log2025-1.rekor.sigstore.dev",
        }:
            matches.append(line)
    expected = (repo / bridge.TRUSTED_ROOT_PATH).read_bytes()
    if matches != [expected]:
        _fail("FR0010_CAPTURE_TRUSTED_ROOT_DRIFT")
    bridge._validate_trusted_root(expected)
    print(
        "framework-recovery-fr-0010-capture: OK "
        f"(TUF Public Good root matches R pin; {version.splitlines()[0]})"
    )


def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description=__doc__)
    subparsers = parser.add_subparsers(dest="command", required=True)
    hosted = subparsers.add_parser("hosted")
    hosted.add_argument(
        "--stage", choices=("repair", "qualification"), required=True
    )
    hosted.add_argument("--workflow", choices=("ci", "formal"), required=True)
    hosted.add_argument("--run-id", type=int, required=True)
    hosted.add_argument("--repair-commit", required=True)
    hosted.add_argument("--subject-commit", required=True)
    c5 = subparsers.add_parser("c5")
    c5.add_argument("--workflow", choices=("ci", "formal"), required=True)
    c5.add_argument("--run-id", type=int, required=True)
    c5.add_argument("--repair-commit", required=True)
    protection = subparsers.add_parser("branch-protection")
    protection.add_argument("--qualification-commit", required=True)
    request = subparsers.add_parser("review-request")
    request.add_argument(
        "--review-id", choices=("FR-0010-R01", "FR-0010-R02"), required=True
    )
    request.add_argument("--repair-commit", required=True)
    request.add_argument("--output", type=Path, required=True)
    review = subparsers.add_parser("review-capture")
    review.add_argument(
        "--review-id", choices=("FR-0010-R01", "FR-0010-R02"), required=True
    )
    review.add_argument("--repair-commit", required=True)
    review.add_argument("--response", type=Path, required=True)
    local = subparsers.add_parser("local")
    local.add_argument("--repair-commit", required=True)
    subparsers.add_parser("verify-root")
    return parser


def main(argv: Sequence[str] | None = None) -> int:
    arguments = _parser().parse_args(argv)
    try:
        repo = _repository()
        bridge = _load_module(
            repo / BRIDGE_PATH, "_haldir_fr0010_capture_bridge"
        )
        protocol = _load_module(
            repo / PROTOCOL_PATH, "_haldir_fr0010_capture_protocol"
        )
        if arguments.command == "hosted":
            _capture_hosted(repo, bridge, protocol, arguments)
        elif arguments.command == "c5":
            _capture_c5(repo, bridge, protocol, arguments)
        elif arguments.command == "branch-protection":
            _capture_branch_protection(repo, bridge, arguments)
        elif arguments.command == "review-request":
            _review_request(repo, bridge, protocol, arguments)
        elif arguments.command == "review-capture":
            _capture_review(repo, bridge, protocol, arguments)
        elif arguments.command == "local":
            _capture_local(repo, bridge, arguments)
        elif arguments.command == "verify-root":
            _verify_root(repo, bridge)
        else:
            _fail("FR0010_CAPTURE_COMMAND")
    except (
        CaptureError,
        OSError,
        UnicodeDecodeError,
        subprocess.TimeoutExpired,
    ) as error:
        print(f"framework-recovery-fr-0010-capture: {error}", file=sys.stderr)
        return 1
    return 0


if __name__ == "__main__":
    raise SystemExit(main())
