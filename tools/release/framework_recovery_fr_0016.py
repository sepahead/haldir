#!/usr/bin/env python3
"""Pure validation primitives for the FR-0016 epoch-17 recovery.
...
RESULT_PROTOCOL = "HALDIR_EPOCH_17_HOSTED_RESULT_V1"
...
EPOCH17_CI_JOB_NAMES = CI_JOB_NAMES | frozenset({"attest-ci-audit-result"})
EPOCH17_FORMAL_JOB_NAMES = FORMAL_JOB_NAMES | frozenset({"attest-formal-audit-result"})
RESULT_CONTRACT = {
    "ci": {
...
EPOCH17_ORDINARY_RUN_FIELDS = ORDINARY_RUN_FIELDS | {
    "number",
    "workflowDatabaseId",
}
EPOCH17_ATTEMPT_RUN_FIELDS = ATTEMPT_RUN_FIELDS | {"number"}
...
class RecoveryProtocolError(ValueError):
    """One fail-closed FR-0016 validation error."""
...
def _require_dict(value: Any, fields: frozenset[str], label: str) -> dict[str, Any]:
    if not isinstance(value, dict) or set(value) != fields:
        _fail("FR0016_FIELDS:" + label)
    return value


def _bounded_int(value: Any, *, minimum: int, maximum: int, label: str) -> int:
    if type(value) is not int or not minimum <= value <= maximum:
        _fail("FR0016_INTEGER:" + label)
    return value
    ...
    *,
    ...
    for index, raw in enumerate(jobs):
        ...
        if (
            ...
            _fail("FR0016_JOB:" + f"{label}.{index}")
        ...
        if started > completed or started < run_start or completed > run_end:
            _fail("FR0016_JOB_TIME:" + f"{label}.{index}")
        steps = [
            _validate_step(
                step,
                run_start=started,
        ...
        if not steps or len(numbers) != len(set(numbers)):
            _fail("FR0016_JOB_STEPS:" + f"{label}.{index}")
        result[name] = {
    ...
    if set(result) != expected_names:
        _fail("FR0016_JOB_SET:" + label)
    return result


def validate_epoch17_run_documents(
    ordinary: Any,
    attempt: Any,
    *,
    workflow: str,
    ...
    """Validate a successful epoch-17 run, including its OIDC attestation job."""
    ...
    if (
        ...
        _fail("FR0016_EPOCH17_RUN_IDENTITY")
    ordinary = _require_dict(ordinary, EPOCH17_ORDINARY_RUN_FIELDS, "epoch17.ordinary")
    attempt = _require_dict(attempt, EPOCH17_ATTEMPT_RUN_FIELDS, "epoch17.attempt")
    run_id = _bounded_int(
        ordinary["databaseId"],
        minimum=1,
        maximum=2**63 - 1,
        label="epoch17.run_id",
    )
    attempt_number = _bounded_int(
        attempt["attempt"],
        minimum=1,
        maximum=MAX_EPOCH17_RUN_ATTEMPT,
        label="epoch17.attempt_number",
    )
    ordinary_attempt = _bounded_int(
        ordinary["attempt"],
        minimum=1,
        maximum=MAX_EPOCH17_RUN_ATTEMPT,
        label="epoch17.ordinary_attempt",
    )
    attempt_run_id = _bounded_int(
        attempt["databaseId"],
        minimum=1,
        maximum=2**63 - 1,
        label="epoch17.attempt_run_id",
    )
    ...
    if (
        ...
        _fail("FR0016_EPOCH17_RUN_METADATA")
    for key in COMMON_RUN_FIELDS - {"createdAt", "jobs", "updatedAt", "url"}:
        if ordinary[key] != attempt[key]:
            _fail("FR0016_EPOCH17_RUN_ATTEMPT_MISMATCH:" + key)
    created = _parse_utc(ordinary["createdAt"], "epoch17.ordinary.created")
    ordinary_updated = _parse_utc(ordinary["updatedAt"], "epoch17.ordinary.updated")
    attempt_created = _parse_utc(attempt["createdAt"], "epoch17.attempt.created")
    attempt_started = _parse_utc(attempt["startedAt"], "epoch17.attempt.started")
    attempt_updated = _parse_utc(attempt["updatedAt"], "epoch17.attempt.updated")
    if (
        ...
        _fail("FR0016_EPOCH17_RUN_CHRONOLOGY")
    expected_jobs = (
        EPOCH17_CI_JOB_NAMES if workflow == "ci" else EPOCH17_FORMAL_JOB_NAMES
    )
    _validate_jobs(
        ordinary["jobs"],
    ...
    if (
        ...
        _fail("FR0016_EPOCH17_CURRENT_ATTEMPT_JOBS")
    return {
        "run_id": run_id,
...
def _validate_pull_request_step(
    ...
    *,
    ...
    if (
        ...
        _fail("FR0016_PULL_REQUEST_STEP:" + label)
    started = _parse_utc(step["startedAt"], label + ".started")
    completed = _parse_utc(step["completedAt"], label + ".completed")
    tolerance = timedelta(seconds=TIMESTAMP_SKEW_SECONDS)
    if (
        ...
        _fail("FR0016_PULL_REQUEST_STEP_TIME:" + label)
    return {
        "number": number,
        "name": step["name"],
...
def _validate_pull_request_jobs(
    ...
    *,
    ...
    if not isinstance(jobs, list) or len(jobs) != len(expected_names):
        _fail("FR0016_PULL_REQUEST_JOBS")
    result: dict[str, dict[str, Any]] = {}
    job_ids: set[int] = set()
    for index, raw in enumerate(jobs):
        ...
        if (
            ...
            _fail("FR0016_PULL_REQUEST_JOB:" + label)
        ...
        if (
            ...
            _fail("FR0016_PULL_REQUEST_JOB_TIME:" + label)
        if name == attestation_name:
            if job["conclusion"] != "skipped" or job["steps"] != []:
                _fail("FR0016_PULL_REQUEST_ATTESTATION_JOB")
            ...
            if (
                ...
                _fail("FR0016_PULL_REQUEST_JOB_STEPS:" + label)
        result[name] = {
            "database_id": database_id,
        ...
        job_ids.add(database_id)
    if set(result) != expected_names:
        _fail("FR0016_PULL_REQUEST_JOB_SET")
    return result
...
def validate_pull_request_run_document(
    ...
    *,
    ...
    if (
        ...
        _fail("FR0016_PULL_REQUEST_RUN_IDENTITY")
    run = _require_dict(value, EPOCH17_ORDINARY_RUN_FIELDS, "pull_request.run")
    run_id = _bounded_int(
        run["databaseId"],
        minimum=1,
        maximum=2**63 - 1,
    ...
    if (
        ...
        _fail("FR0016_PULL_REQUEST_RUN_METADATA")
...
def _validate_file_record(value: Any, *, expected_path: str) -> dict[str, Any]:
    ...
    if (
        ...
        _fail("FR0016_FILE_RECORD:" + expected_path)
...
def validate_result_artifact(
    ...
    *,
    ...
    if (
        ...
        _fail("FR0016_RESULT_BOUND")
    try:
        value = json.loads(payload)
    except (UnicodeDecodeError, json.JSONDecodeError):
        _fail("FR0016_RESULT_JSON")
    if (
    ...
    if (
        ...
        _fail("FR0016_RESULT_REPOSITORY")
    ...
    if canonical_json_bytes(normalized_observed) != canonical_json_bytes(
        ...
        _fail("FR0016_RESULT_MATERIAL_DRIFT")
    expected_authority = {
...
def validate_artifact_metadata(
    ...
    *,
    ...
    if workflow not in RESULT_CONTRACT:
        _fail("FR0016_ARTIFACT_WORKFLOW")
    ...
    if (
        ...
        _fail("FR0016_ARTIFACT_IDENTITY")
    ...
    if (
        ...
        _fail("FR0016_ARTIFACT_CHRONOLOGY")
    ...
    if (
        ...
        _fail("FR0016_ARTIFACT_RUN_BINDING")
...
def _parse_attestation_bundle(payload: bytes) -> dict[str, Any]:
    if (
        ...
        _fail("FR0016_ATTESTATION_BUNDLE_BOUND")
    try:
        bundle = json.loads(payload)
    except (UnicodeDecodeError, json.JSONDecodeError):
        _fail("FR0016_ATTESTATION_BUNDLE_JSON")
    if (
        ...
        _fail("FR0016_ATTESTATION_BUNDLE_SCHEMA")
    ...
    if (
        ...
        _fail("FR0016_ATTESTATION_ENVELOPE")
    try:
        ...
        _fail("FR0016_ATTESTATION_STATEMENT")
...
def _expected_attestation_statement(
    *,
    ...
    return {
        ...
        "subject": [
            {
                "name": (f"epoch-17-{workflow}-result-attempt-{attempt}.json"),
...
def validate_attestation_evidence(
    ...
    *,
    ...
    if (
        ...
        or run_id < 1
        or type(attempt) is not int
        or not 1 <= attempt <= MAX_EPOCH17_RUN_ATTEMPT
    ):
        _fail("FR0016_ATTESTATION_IDENTITY")
    ...
    if canonical_json_bytes(parsed["statement"]) != canonical_json_bytes(
        ...
        _fail("FR0016_ATTESTATION_STATEMENT_MISMATCH")
    if (
        ...
        _fail("FR0016_ATTESTATION_RECEIPT")
    ...
    if (
        ...
        _fail("FR0016_ATTESTATION_RECEIPT_BUNDLE")
    ...
    if (
        ...
        _fail("FR0016_ATTESTATION_VERIFICATION_RESULT")
    ...
    if (
        ...
        _fail("FR0016_ATTESTATION_TRANSPARENCY_LOG")
    ...
    if (
        ...
        _fail("FR0016_ATTESTATION_TRANSPARENCY_LOG_TIME")
    ...
    if not isinstance(certificate, dict) or any(
        ...
        _fail("FR0016_ATTESTATION_CERTIFICATE")
    return {
