#!/usr/bin/env python3
"""Pure validation primitives for the FR-0013 epoch-14 recovery.
...
RESULT_PROTOCOL = "HALDIR_EPOCH_14_HOSTED_RESULT_V1"
...
EPOCH14_CI_JOB_NAMES = CI_JOB_NAMES | frozenset({"attest-ci-audit-result"})
EPOCH14_FORMAL_JOB_NAMES = FORMAL_JOB_NAMES | frozenset({"attest-formal-audit-result"})
RESULT_CONTRACT = {
    "ci": {
...
EPOCH14_ORDINARY_RUN_FIELDS = ORDINARY_RUN_FIELDS | {
    "number",
    "workflowDatabaseId",
}
EPOCH14_ATTEMPT_RUN_FIELDS = ATTEMPT_RUN_FIELDS | {"number"}
...
class RecoveryProtocolError(ValueError):
    """One fail-closed FR-0013 validation error."""
...
def _require_dict(value: Any, fields: frozenset[str], label: str) -> dict[str, Any]:
    if not isinstance(value, dict) or set(value) != fields:
        _fail("FR0013_FIELDS:" + label)
    return value


def _bounded_int(value: Any, *, minimum: int, maximum: int, label: str) -> int:
    if type(value) is not int or not minimum <= value <= maximum:
        _fail("FR0013_INTEGER:" + label)
    return value
    ...
    *,
    ...
    for index, raw in enumerate(jobs):
        ...
        if (
            ...
            _fail("FR0013_JOB:" + f"{label}.{index}")
        ...
        if started > completed or started < run_start or completed > run_end:
            _fail("FR0013_JOB_TIME:" + f"{label}.{index}")
        steps = [
            _validate_step(
                step,
                run_start=started,
        ...
        if not steps or len(numbers) != len(set(numbers)):
            _fail("FR0013_JOB_STEPS:" + f"{label}.{index}")
        result[name] = {
    ...
    if set(result) != expected_names:
        _fail("FR0013_JOB_SET:" + label)
    return result


def validate_epoch14_run_documents(
    ordinary: Any,
    attempt: Any,
    *,
    workflow: str,
    ...
    """Validate a successful epoch-14 run, including its OIDC attestation job."""
    ...
    if (
        ...
        _fail("FR0013_EPOCH14_RUN_IDENTITY")
    ordinary = _require_dict(ordinary, EPOCH14_ORDINARY_RUN_FIELDS, "epoch14.ordinary")
    attempt = _require_dict(attempt, EPOCH14_ATTEMPT_RUN_FIELDS, "epoch14.attempt")
    run_id = _bounded_int(
        ordinary["databaseId"],
        minimum=1,
        maximum=2**63 - 1,
        label="epoch14.run_id",
    )
    attempt_number = _bounded_int(
        attempt["attempt"],
        minimum=1,
        maximum=MAX_EPOCH14_RUN_ATTEMPT,
        label="epoch14.attempt_number",
    )
    ordinary_attempt = _bounded_int(
        ordinary["attempt"],
        minimum=1,
        maximum=MAX_EPOCH14_RUN_ATTEMPT,
        label="epoch14.ordinary_attempt",
    )
    attempt_run_id = _bounded_int(
        attempt["databaseId"],
        minimum=1,
        maximum=2**63 - 1,
        label="epoch14.attempt_run_id",
    )
    ...
    if (
        ...
        _fail("FR0013_EPOCH14_RUN_METADATA")
    for key in COMMON_RUN_FIELDS - {"createdAt", "jobs", "updatedAt", "url"}:
        if ordinary[key] != attempt[key]:
            _fail("FR0013_EPOCH14_RUN_ATTEMPT_MISMATCH:" + key)
    created = _parse_utc(ordinary["createdAt"], "epoch14.ordinary.created")
    ordinary_updated = _parse_utc(ordinary["updatedAt"], "epoch14.ordinary.updated")
    attempt_created = _parse_utc(attempt["createdAt"], "epoch14.attempt.created")
    attempt_started = _parse_utc(attempt["startedAt"], "epoch14.attempt.started")
    attempt_updated = _parse_utc(attempt["updatedAt"], "epoch14.attempt.updated")
    if (
        ...
        _fail("FR0013_EPOCH14_RUN_CHRONOLOGY")
    expected_jobs = (
        EPOCH14_CI_JOB_NAMES if workflow == "ci" else EPOCH14_FORMAL_JOB_NAMES
    )
    _validate_jobs(
        ordinary["jobs"],
    ...
    if (
        ...
        _fail("FR0013_EPOCH14_CURRENT_ATTEMPT_JOBS")
    return {
        "run_id": run_id,
...
def _validate_file_record(value: Any, *, expected_path: str) -> dict[str, Any]:
    ...
    if (
        ...
        _fail("FR0013_FILE_RECORD:" + expected_path)
...
def validate_result_artifact(
    ...
    *,
    ...
    if (
        ...
        _fail("FR0013_RESULT_BOUND")
    try:
        value = json.loads(payload)
    except (UnicodeDecodeError, json.JSONDecodeError):
        _fail("FR0013_RESULT_JSON")
    if (
    ...
    if (
        ...
        _fail("FR0013_RESULT_REPOSITORY")
    ...
    if canonical_json_bytes(normalized_observed) != canonical_json_bytes(
        ...
        _fail("FR0013_RESULT_MATERIAL_DRIFT")
    expected_authority = {
...
def validate_artifact_metadata(
    ...
    *,
    ...
    if workflow not in RESULT_CONTRACT:
        _fail("FR0013_ARTIFACT_WORKFLOW")
    ...
    if (
        ...
        _fail("FR0013_ARTIFACT_IDENTITY")
    ...
    if (
        ...
        _fail("FR0013_ARTIFACT_CHRONOLOGY")
    ...
    if (
        ...
        _fail("FR0013_ARTIFACT_RUN_BINDING")
...
def _parse_attestation_bundle(payload: bytes) -> dict[str, Any]:
    if (
        ...
        _fail("FR0013_ATTESTATION_BUNDLE_BOUND")
    try:
        bundle = json.loads(payload)
    except (UnicodeDecodeError, json.JSONDecodeError):
        _fail("FR0013_ATTESTATION_BUNDLE_JSON")
    if (
        ...
        _fail("FR0013_ATTESTATION_BUNDLE_SCHEMA")
    ...
    if (
        ...
        _fail("FR0013_ATTESTATION_ENVELOPE")
    try:
        ...
        _fail("FR0013_ATTESTATION_STATEMENT")
...
def _expected_attestation_statement(
    *,
    ...
    return {
        ...
        "subject": [
            {
                "name": (f"epoch-14-{workflow}-result-attempt-{attempt}.json"),
...
def validate_attestation_evidence(
    ...
    *,
    ...
    if (
        ...
        or run_id < 1
        or type(attempt) is not int
        or not 1 <= attempt <= MAX_EPOCH14_RUN_ATTEMPT
    ):
        _fail("FR0013_ATTESTATION_IDENTITY")
    ...
    if canonical_json_bytes(parsed["statement"]) != canonical_json_bytes(
        ...
        _fail("FR0013_ATTESTATION_STATEMENT_MISMATCH")
    if (
        ...
        _fail("FR0013_ATTESTATION_RECEIPT")
    ...
    if (
        ...
        _fail("FR0013_ATTESTATION_RECEIPT_BUNDLE")
    ...
    if (
        ...
        _fail("FR0013_ATTESTATION_VERIFICATION_RESULT")
    ...
    if (
        ...
        _fail("FR0013_ATTESTATION_TRANSPARENCY_LOG")
    ...
    if (
        ...
        _fail("FR0013_ATTESTATION_TRANSPARENCY_LOG_TIME")
    ...
    if not isinstance(certificate, dict) or any(
        ...
        _fail("FR0013_ATTESTATION_CERTIFICATE")
    return {
