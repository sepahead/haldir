#!/usr/bin/env python3
"""Offline adversarial tests for the FR-0016 epoch-17 trust root."""
...
PROTOCOL = load_module(
    ...
    "_haldir_fr0016_test_protocol",
)
RESULT = load_module(
    "tools/release/framework_recovery_fr_0016_result.py",
    "_haldir_fr0016_test_result",
)
BRIDGE = load_module(
    "tools/release/verify-framework-recovery-fr-0016.py",
    "_haldir_fr0016_test_bridge",
)
CAPTURE = load_module(
    "tools/release/framework_recovery_fr_0016_capture.py",
    "_haldir_fr0016_test_capture",
)
PINS = load_module("tools/verify-ci-pins.py", "_haldir_fr0016_test_pins")
...
def run_documents(workflow: str = "ci") -> tuple[dict, dict]:
    if workflow == "ci":
        names = sorted(PROTOCOL.EPOCH17_CI_JOB_NAMES)
        producer = "supply-chain"
        attester = "attest-ci-audit-result"
    else:
        names = sorted(PROTOCOL.EPOCH17_FORMAL_JOB_NAMES)
...
class RunDocumentTests(unittest.TestCase):
    def validate(
        ...
        *,
        ...
        return PROTOCOL.validate_epoch17_run_documents(
            ordinary,
            attempt,
            workflow=workflow,
            subject_commit=COMMIT,
    ...
    def test_attempt_two_is_valid_and_fully_bound(self) -> None:
        ...
        self.assertEqual(set(result["jobs"]), PROTOCOL.EPOCH17_CI_JOB_NAMES)
        ...
        for name in ("supply-chain", "attest-ci-audit-result"):
            ...
            with (
                ...
                self.assertRaisesRegex(
                    ValueError,
                    "FR0016_EPOCH17_CURRENT_ATTEMPT_JOBS",
                ),
            ):
                self.validate(ordinary, attempt)
    ...
    def test_attempt_one_cannot_start_before_original_run(self) -> None:
        ...
        with self.assertRaisesRegex(ValueError, "EPOCH17_RUN_CHRONOLOGY"):
            self.validate(ordinary, attempt)
    ...
    def test_attempt_one_job_cannot_start_before_original_run(self) -> None:
        ...
        with self.assertRaisesRegex(ValueError, "FR0016_JOB_TIME"):
            self.validate(ordinary, attempt)
    ...
    def test_noncritical_job_outer_bounds_are_exact(self) -> None:
        ...
        for label, started_at, completed_at in attacks:
            ...
            with (
                ...
                self.assertRaisesRegex(ValueError, "FR0016_JOB_TIME"),
            ):
                self.validate(ordinary, attempt)
    ...
    def test_cross_run_job_url_is_rejected(self) -> None:
        ...
        with self.assertRaisesRegex(ValueError, "FR0016_JOB"):
            PROTOCOL.validate_epoch17_run_documents(
                ordinary,
                attempt,
                workflow="ci",
                subject_commit=COMMIT,
    ...
    def test_cross_attempt_documents_are_rejected(self) -> None:
        ordinary, attempt = run_documents()
        ordinary["attempt"] = 1
        with self.assertRaisesRegex(ValueError, "EPOCH17_RUN_METADATA"):
            PROTOCOL.validate_epoch17_run_documents(
                ordinary,
                attempt,
                workflow="ci",
    ...
    def test_ordinary_attempt_job_mismatch_is_rejected(self) -> None:
        ...
        with self.assertRaisesRegex(ValueError, "EPOCH17_RUN_JOB_MISMATCH"):
            self.validate(ordinary, attempt)
            ...
            for invalid in (True, float(valid)):
                for side in ("ordinary", "attempt"):
                    ...
                    with (
                        ...
                        self.assertRaisesRegex(ValueError, "FR0016_INTEGER"),
                    ):
                        self.validate(ordinary, attempt)
    ...
    def test_step_one_second_tolerance_is_exact(self) -> None:
        ...
        with self.assertRaisesRegex(ValueError, "FR0016_STEP_TIME"):
            self.validate(ordinary, attempt)
    ...
    def test_attempt_bound_is_fail_closed(self) -> None:
        ...
        with self.assertRaisesRegex(ValueError, "epoch17.attempt_number"):
            PROTOCOL.validate_epoch17_run_documents(
                ordinary,
                attempt,
                workflow="ci",
...
class PlanContractTests(unittest.TestCase):
    ...
    def test_plan_names_the_four_epoch_17_defect_classes(self) -> None:
        plan = self.plan()
        self.assertEqual(
            [defect["id"] for defect in plan["defects"]],
            ["FR0016-D01", "FR0016-D02", "FR0016-D03", "FR0016-D04"],
        )
        summaries = " ".join(defect["summary"] for defect in plan["defects"])
        for term in (
            "two-parent synthetic pull-request merge commit",
            "cancel-in-progress for main",
            "exact Temurin archive bytes",
            "pipes Java through tee",
            "portable local architecture vocabulary",
        ):
            with self.subTest(term=term):
                self.assertIn(term, summaries)
        self.assertNotIn("cargo-deny Docker Action", summaries)
    ...
    def test_plan_closes_pr_concurrency_and_formal_runtime_contracts(self) -> None:
        ...
        self.assertFalse(pull_request["epoch_17_recovery_primitive_suite_skipped"])
    ...
    def test_plan_binds_epoch_17_transport_and_exact_hosted_policy(self) -> None:
        correction = self.plan()["correction"]
        self.assertEqual(
        ...
        for label, path, valid in mutations:
            for invalid in (True, float(valid)):
                ...
                with (
                    ...
                    self.assertRaisesRegex(ValueError, "FR0016_"),
                ):
...
class TrustedRootAndCommandTests(unittest.TestCase):
    def test_signed_fr0015_active_boundary_is_verified_directly(self) -> None:
        BRIDGE._verify_legacy_boundary(ROOT)

    def test_real_openssh_detached_signature_is_accepted(self) -> None:
        payload = b"FR-0016 detached signature parser integration\n"
        namespace = "haldir-fr0016-openssh-integration"
    ...
    def test_source_authority_file_is_frozen(self) -> None:
        self.assertIn(
            ...
            BRIDGE.FR0015_BOUNDARY_RECORDS,
        )
        self.assertIn(
            BRIDGE.ALLOWED_SIGNERS_PATH,
        ...
        with tempfile.TemporaryDirectory() as name:
            ...
            for mode in (0o755, 0o666):
                with self.subTest(mode=oct(mode)):
                    ...
                    with self.assertRaisesRegex(RuntimeError, "FR0016_WORKTREE"):
                        BRIDGE._verify_worktree(repo, commit, ["protected.txt"])
...
class WorkflowAndPinTests(unittest.TestCase):
    def test_epoch17_authority_cannot_inherit_epoch16(self) -> None:
        authority = BRIDGE._authority("ACTIVE")
        self.assertEqual(authority["framework_epoch"], 17)
        stale = copy.deepcopy(authority)
        stale["framework_epoch"] = 16
    ...
    def test_rqa_scopes_are_exact_append_only(self) -> None:
        self.assertEqual(
            BRIDGE.REPAIR_STATUSES,
            {
                ...
                BRIDGE.SOURCE_PIN_VERIFIER_PATH: "M",
                BRIDGE.SUPPLY_PIN_PATH: "M",
                BRIDGE.CARGO_DENY_INSTALLER_PATH: "M",
                BRIDGE.CARGO_DENY_TEST_PATH: "M",
                BRIDGE.FORMAL_RUNNER_PATH: "M",
                BRIDGE.FORMAL_RUNNER_TEST_PATH: "M",
                BRIDGE.FORMAL_README_PATH: "M",
                BRIDGE.JUSTFILE_PATH: "M",
                BRIDGE.GATE_PATH: "M",
        ...
        self.assertEqual(
            ...
            "haldir-framework-recovery-fr-0016-plan-v1",
        )
        self.assertEqual(
            BRIDGE.QUALIFICATION_NAMESPACE,
            "haldir-framework-recovery-fr-0016-qualification-v1",
        )
        self.assertEqual(
            BRIDGE.ACTIVATION_NAMESPACE,
            "haldir-framework-recovery-fr-0016-activation-v1",
        )
        self.assertEqual(
    ...
    def test_attempt_qualified_unarchived_outputs(self) -> None:
        for workflow in ("ci", "formal"):
            ...
            self.assertIn(
                f"epoch-17-{workflow}-result-attempt-${{{{ github.run_attempt }}}}.json",
                text,
            )
            self.assertNotIn(f"name: epoch-17-{workflow}-result-attempt", text)
    ...
    def test_gate_does_not_execute_legacy_verifier(self) -> None:
        ...
        self.assertIn("verify-framework-recovery-fr-0016.py", text)
    ...
    def test_new_root_never_imports_fr0010_through_fr0015_python(self) -> None:
        legacy_modules = (
...
class CapturePrimitiveTests(unittest.TestCase):
    ...
    def test_epoch17_field_contracts(self) -> None:
        self.assertEqual(
            CAPTURE._run_fields(epoch17=True, attempt=False),
            (
        ...
        self.assertIn(
            "startedAt",
            CAPTURE._run_fields(epoch17=True, attempt=True),
        )
