#!/usr/bin/env python3
"""Offline adversarial tests for the FR-0015 epoch-16 trust root."""
...
PROTOCOL = load_module(
    ...
    "_haldir_fr0015_test_protocol",
)
RESULT = load_module(
    "tools/release/framework_recovery_fr_0015_result.py",
    "_haldir_fr0015_test_result",
)
BRIDGE = load_module(
    "tools/release/verify-framework-recovery-fr-0015.py",
    "_haldir_fr0015_test_bridge",
)
CAPTURE = load_module(
    "tools/release/framework_recovery_fr_0015_capture.py",
    "_haldir_fr0015_test_capture",
)
PINS = load_module("tools/verify-ci-pins.py", "_haldir_fr0015_test_pins")
...
def run_documents(workflow: str = "ci") -> tuple[dict, dict]:
    if workflow == "ci":
        names = sorted(PROTOCOL.EPOCH16_CI_JOB_NAMES)
        producer = "supply-chain"
        attester = "attest-ci-audit-result"
    else:
        names = sorted(PROTOCOL.EPOCH16_FORMAL_JOB_NAMES)
...
class RunDocumentTests(unittest.TestCase):
    def validate(
        ...
        *,
        ...
        return PROTOCOL.validate_epoch16_run_documents(
            ordinary,
            attempt,
            workflow=workflow,
            subject_commit=COMMIT,
    ...
    def test_attempt_two_is_valid_and_fully_bound(self) -> None:
        ...
        self.assertEqual(set(result["jobs"]), PROTOCOL.EPOCH16_CI_JOB_NAMES)
        ...
        for name in ("supply-chain", "attest-ci-audit-result"):
            ...
            with (
                ...
                self.assertRaisesRegex(
                    ValueError,
                    "FR0015_EPOCH16_CURRENT_ATTEMPT_JOBS",
                ),
            ):
                self.validate(ordinary, attempt)
    ...
    def test_attempt_one_cannot_start_before_original_run(self) -> None:
        ...
        with self.assertRaisesRegex(ValueError, "EPOCH16_RUN_CHRONOLOGY"):
            self.validate(ordinary, attempt)
    ...
    def test_attempt_one_job_cannot_start_before_original_run(self) -> None:
        ...
        with self.assertRaisesRegex(ValueError, "FR0015_JOB_TIME"):
            self.validate(ordinary, attempt)
    ...
    def test_noncritical_job_outer_bounds_are_exact(self) -> None:
        ...
        for label, started_at, completed_at in attacks:
            ...
            with (
                ...
                self.assertRaisesRegex(ValueError, "FR0015_JOB_TIME"),
            ):
                self.validate(ordinary, attempt)
    ...
    def test_cross_run_job_url_is_rejected(self) -> None:
        ...
        with self.assertRaisesRegex(ValueError, "FR0015_JOB"):
            PROTOCOL.validate_epoch16_run_documents(
                ordinary,
                attempt,
                workflow="ci",
                subject_commit=COMMIT,
    ...
    def test_cross_attempt_documents_are_rejected(self) -> None:
        ordinary, attempt = run_documents()
        ordinary["attempt"] = 1
        with self.assertRaisesRegex(ValueError, "EPOCH16_RUN_METADATA"):
            PROTOCOL.validate_epoch16_run_documents(
                ordinary,
                attempt,
                workflow="ci",
    ...
    def test_ordinary_attempt_job_mismatch_is_rejected(self) -> None:
        ...
        with self.assertRaisesRegex(ValueError, "EPOCH16_RUN_JOB_MISMATCH"):
            self.validate(ordinary, attempt)
            ...
            for invalid in (True, float(valid)):
                for side in ("ordinary", "attempt"):
                    ...
                    with (
                        ...
                        self.assertRaisesRegex(ValueError, "FR0015_INTEGER"),
                    ):
                        self.validate(ordinary, attempt)
    ...
    def test_step_one_second_tolerance_is_exact(self) -> None:
        ...
        with self.assertRaisesRegex(ValueError, "FR0015_STEP_TIME"):
            self.validate(ordinary, attempt)
    ...
    def test_attempt_bound_is_fail_closed(self) -> None:
        ...
        with self.assertRaisesRegex(ValueError, "epoch16.attempt_number"):
            PROTOCOL.validate_epoch16_run_documents(
                ordinary,
                attempt,
                workflow="ci",
        ...
        for label, path, valid in mutations:
            for invalid in (True, float(valid)):
                ...
                with (
                    ...
                    self.assertRaisesRegex(ValueError, "FR0015_"),
                ):
...
class TrustedRootAndCommandTests(unittest.TestCase):
    def test_signed_fr0014_active_boundary_is_verified_directly(self) -> None:
        BRIDGE._verify_legacy_boundary(ROOT)

    def test_real_openssh_detached_signature_is_accepted(self) -> None:
        payload = b"FR-0015 detached signature parser integration\n"
        namespace = "haldir-fr0015-openssh-integration"
    ...
    def test_source_authority_file_is_frozen(self) -> None:
        self.assertIn(
            ...
            BRIDGE.FR0014_BOUNDARY_RECORDS,
        )
        self.assertIn(
            BRIDGE.ALLOWED_SIGNERS_PATH,
        ...
        with tempfile.TemporaryDirectory() as name:
            ...
            for mode in (0o755, 0o666):
                with self.subTest(mode=oct(mode)):
                    ...
                    with self.assertRaisesRegex(RuntimeError, "FR0015_WORKTREE"):
                        BRIDGE._verify_worktree(repo, commit, ["protected.txt"])
...
class WorkflowAndPinTests(unittest.TestCase):
    def test_rqa_scopes_are_exact_append_only(self) -> None:
        ...
        self.assertEqual(
            BRIDGE.FR0014_PLAN_NAMESPACE,
            "haldir-framework-recovery-fr-0014-plan-v1",
        )
        self.assertEqual(
            BRIDGE.FR0014_QUALIFICATION_NAMESPACE,
            "haldir-framework-recovery-fr-0014-qualification-v1",
        )
        self.assertEqual(
            BRIDGE.FR0014_ACTIVATION_NAMESPACE,
    ...
    def test_attempt_qualified_unarchived_outputs(self) -> None:
        for workflow in ("ci", "formal"):
            ...
            self.assertIn(
                f"epoch-16-{workflow}-result-attempt-${{{{ github.run_attempt }}}}.json",
                text,
            )
            self.assertNotIn(f"name: epoch-16-{workflow}-result-attempt", text)
    ...
    def test_gate_does_not_execute_legacy_verifier(self) -> None:
        ...
        self.assertIn("verify-framework-recovery-fr-0015.py", text)
    ...
    def test_new_root_never_imports_fr0010_through_fr0014_python(self) -> None:
        legacy_modules = (
...
class CapturePrimitiveTests(unittest.TestCase):
    ...
    def test_epoch16_field_contracts(self) -> None:
        self.assertEqual(
            CAPTURE._run_fields(epoch16=True, attempt=False),
            (
        ...
        self.assertIn(
            "startedAt",
            CAPTURE._run_fields(epoch16=True, attempt=True),
        )
