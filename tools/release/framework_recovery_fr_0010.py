#!/usr/bin/env python3
"""Pure validation primitives for the FR-0010 epoch-11 recovery.
...
Bounded GitHub whole-job archives are accepted only to reproduce the c5/FR-0009
defect; they are not an epoch-11 PASS oracle.
"""
...
C5_ARCHIVE_PROTOCOL = "HALDIR_FR_0010_C5_WHOLE_JOB_ARCHIVE_V1"
RESULT_PROTOCOL = "HALDIR_EPOCH_11_HOSTED_RESULT_V1"
REVIEW_REQUEST_PROTOCOL = "HALDIR_FR_0010_REVIEW_REQUEST_V1"
REVIEW_RESPONSE_PROTOCOL = "HALDIR_FR_0010_REVIEW_RESPONSE_V1"
...
MAX_EPOCH11_RUN_ATTEMPT = 8
HEX40 = re.compile(r"^[0-9a-f]{40}$")
HEX64 = re.compile(r"^[0-9a-f]{64}$")
JOB_NAME = re.compile(r"^[a-z0-9][a-z0-9-]{0,62}$")
ARCHIVE_JOB = re.compile(r"^(?P<index>0|[1-9][0-9]*)_(?P<job>[a-z0-9][a-z0-9-]{0,62})\.txt$")
LOG_LINE = re.compile(
    rb"^(?P<timestamp>[0-9]{4}-[0-9]{2}-[0-9]{2}T"
    rb"[0-9]{2}:[0-9]{2}:[0-9]{2}(?:\.[0-9]{1,9})?Z) "
    rb"(?P<body>[^\r\n]*)$"
)
RUN_MARKER = re.compile(rb"^Ran (?P<count>0|[1-9][0-9]*) tests? in [0-9]+(?:\.[0-9]+)?s$")
...
EPOCH11_CI_JOB_NAMES = CI_JOB_NAMES | frozenset({"attest-ci-audit-result"})
EPOCH11_FORMAL_JOB_NAMES = FORMAL_JOB_NAMES | frozenset(
...
EPOCH11_ORDINARY_RUN_FIELDS = ORDINARY_RUN_FIELDS | {
    "number",
    "workflowDatabaseId",
}
EPOCH11_ATTEMPT_RUN_FIELDS = ATTEMPT_RUN_FIELDS | {"number"}
...
class RecoveryProtocolError(ValueError):
    """One fail-closed FR-0010 validation error."""
...
def _require_dict(value: Any, fields: frozenset[str], label: str) -> dict[str, Any]:
    if not isinstance(value, dict) or set(value) != fields:
        _fail("FR0010_FIELDS:" + label)
    return value


def _bounded_int(value: Any, *, minimum: int, maximum: int, label: str) -> int:
    if type(value) is not int or not minimum <= value <= maximum:
        _fail("FR0010_INTEGER:" + label)
    return value
    ...
    *,
    ...
    for index, raw in enumerate(jobs):
        ...
        if (
            ...
            _fail("FR0010_JOB:" + f"{label}.{index}")
        ...
        if (
            ...
            _fail("FR0010_JOB_TIME:" + f"{label}.{index}")
        steps = [
            _validate_step(
                step,
                run_start=started,
        ...
        if not steps or len(numbers) != len(set(numbers)):
            _fail("FR0010_JOB_STEPS:" + f"{label}.{index}")
        result[name] = {
...
def validate_c5_run_documents(
    ...
    *,
    ...
    if (
        ...
        or type(attempt["workflowDatabaseId"]) is not int
        or not 1 <= attempt["workflowDatabaseId"] <= 2**63 - 1
...
def validate_epoch11_run_documents(
    ordinary: Any,
    attempt: Any,
    *,
    workflow: str,
    ...
    """Validate a successful epoch-11 run, including its OIDC attestation job."""
    ...
    if (
        ...
        _fail("FR0010_EPOCH11_RUN_IDENTITY")
    ordinary = _require_dict(
        ordinary, EPOCH11_ORDINARY_RUN_FIELDS, "epoch11.ordinary"
    )
    attempt = _require_dict(
        attempt, EPOCH11_ATTEMPT_RUN_FIELDS, "epoch11.attempt"
    )
    run_id = _bounded_int(
        ordinary["databaseId"],
        minimum=1,
        maximum=2**63 - 1,
        label="epoch11.run_id",
    )
    attempt_number = _bounded_int(
        attempt["attempt"],
        minimum=1,
        maximum=MAX_EPOCH11_RUN_ATTEMPT,
        label="epoch11.attempt_number",
    )
    ...
    if (
        ...
        _fail("FR0010_EPOCH11_RUN_METADATA")
    for key in COMMON_RUN_FIELDS - {"createdAt", "jobs", "updatedAt", "url"}:
        if ordinary[key] != attempt[key]:
            _fail("FR0010_EPOCH11_RUN_ATTEMPT_MISMATCH:" + key)
    created = _parse_utc(ordinary["createdAt"], "epoch11.ordinary.created")
    ordinary_updated = _parse_utc(
        ordinary["updatedAt"], "epoch11.ordinary.updated"
    )
    attempt_created = _parse_utc(
        attempt["createdAt"], "epoch11.attempt.created"
    )
    attempt_started = _parse_utc(
        attempt["startedAt"], "epoch11.attempt.started"
    )
    attempt_updated = _parse_utc(
        attempt["updatedAt"], "epoch11.attempt.updated"
    )
    if (
        created > attempt_started
        or attempt_created > attempt_started + timedelta(seconds=1)
        ...
        _fail("FR0010_EPOCH11_RUN_CHRONOLOGY")
    expected_jobs = (
        EPOCH11_CI_JOB_NAMES
        if workflow == "ci"
        else EPOCH11_FORMAL_JOB_NAMES
    )
    _validate_jobs(
        ordinary["jobs"],
    ...
    attempt_jobs = _validate_jobs(
        ...
        run_start=attempt_started,
        run_end=attempt_updated,
        label="epoch11.attempt.jobs",
    )
    if canonical_json_bytes(ordinary["jobs"]) != canonical_json_bytes(
        attempt["jobs"]
    ):
        _fail("FR0010_EPOCH11_RUN_JOB_MISMATCH")
    attestation_job = (
        "attest-ci-audit-result"
        if workflow == "ci"
        else "attest-formal-audit-result"
    )
    if attestation_job not in attempt_jobs:
        _fail("FR0010_EPOCH11_ATTESTATION_JOB")
    return {
        "run_id": run_id,
        "attempt": attempt_number,
    ...
    with archive:
        ...
        for name in sorted(names):
            ...
            try:
                ...
                _fail("FR0010_ARCHIVE_READ")
            if len(data) != info.file_size or trailing:
                _fail("FR0010_ARCHIVE_SIZE")
            records.append(
                {
                    "name": name,
...
def _timestamped_lines(payload: bytes, *, label: str) -> list[tuple[datetime, bytes]]:
    if type(payload) is not bytes or not payload or b"\0" in payload or b"\r" in payload:
        _fail("FR0010_LOG_GRAMMAR:" + label)
...
def _command_group_lines(
    ...
    *,
    ...
    start_indexes = [
        index for index, (_timestamp, body) in enumerate(timestamped) if body == group_start
    ]
    if len(start_indexes) != 1:
        _fail("FR0010_COMMAND_GROUP_START:" + label)
    ...
    if not following_groups or following_groups[0][1] != next_group_start:
        _fail("FR0010_COMMAND_GROUP_END:" + label)
...
def validate_c5_ci_archive(
    ...
    *,
    ...
    if (
        ...
        or any(type(count) is not int or not 1 <= count <= 100_000 for count in suite_counts)
    ):
        _fail("FR0010_SUITE_COUNTS")
    ...
    if any(token in line for token in FAILURE_TOKENS for line in lines):
        _fail("FR0010_CI_FAILURE_TOKEN")
    expected = [
        marker
        for count in suite_counts
        for marker in (("RUN", count), ("OK", None))
    ] + [("VERIFIER_OK", None)]
    if _ci_markers(lines) != expected:
        _fail("FR0010_CI_MARKERS")
    return {
...
def validate_c5_formal_archive(
    ...
    *,
    ...
    if (
        ...
        or joined.count(b"Model checking completed. No error has been found.") != 1
        or joined.count(b"Finished in") < 1
    ):
        _fail("FR0010_FORMAL_MARKERS")
    return {
        "archive_sha256": inspected["sha256"],
        "archive_bytes": inspected["bytes"],
        "entry_records": inspected["entries"],
...
def validate_c5_hosted_capture(
    ...
    *,
    ...
    """Validate one diagnostic c5 capture without treating logs as epoch-11 proof."""
    ...
    if canonical_json_bytes(document["archive"]) != canonical_json_bytes(expected_archive):
        _fail("FR0010_HOSTED_ARCHIVE_RECEIPT")
    ...
    if (
        tool["path"] != "tools/release/framework_recovery_fr_0010_capture.py"
    ...
    if run["updated"] > started or started > completed:
        _fail("FR0010_CAPTURE_CHRONOLOGY")
    return {
        "run_id": run_id,
        "attempt": attempt_number,
        "workflow": workflow,
...
def _validate_file_record(value: Any, *, expected_path: str) -> dict[str, Any]:
    ...
    if (
        ...
        _fail("FR0010_FILE_RECORD:" + expected_path)
...
def validate_result_artifact(
    ...
    *,
    ...
    if (
        ...
        _fail("FR0010_RESULT_BOUND")
    try:
        value = json.loads(payload)
    except (UnicodeDecodeError, json.JSONDecodeError):
        _fail("FR0010_RESULT_JSON")
    if (
    ...
    if (
        ...
        _fail("FR0010_RESULT_EXECUTION")
    if (
        not isinstance(expected_materials, list)
        or len(expected_materials) != len(contract["material_paths"])
    ):
        _fail("FR0010_RESULT_EXPECTED_MATERIALS")
    normalized_expected = [
    ...
    if (
        ...
        or len(materials) != len(normalized_expected)
    ):
        _fail("FR0010_RESULT_MATERIALS")
    normalized_observed = [
        _validate_file_record(item, expected_path=path)
        for item, path in zip(
            materials, contract["material_paths"], strict=True
        )
    ]
    if canonical_json_bytes(normalized_observed) != canonical_json_bytes(
        normalized_expected
    ):
        _fail("FR0010_RESULT_MATERIAL_DRIFT")
    if value["authority"] != {
...
def validate_artifact_metadata(
    ...
    *,
    ...
    if workflow not in RESULT_CONTRACT:
        _fail("FR0010_ARTIFACT_WORKFLOW")
    ...
    if (
        ...
        _fail("FR0010_ARTIFACT_IDENTITY")
    ...
    if (
        ...
        _fail("FR0010_ARTIFACT_CHRONOLOGY")
    ...
    if workflow_run != {
        ...
        _fail("FR0010_ARTIFACT_RUN_BINDING")
...
def _parse_attestation_bundle(payload: bytes) -> dict[str, Any]:
    if (
        ...
        _fail("FR0010_ATTESTATION_BUNDLE_BOUND")
    try:
        bundle = json.loads(payload)
    except (UnicodeDecodeError, json.JSONDecodeError):
        _fail("FR0010_ATTESTATION_BUNDLE_JSON")
    if (
        ...
        _fail("FR0010_ATTESTATION_BUNDLE_SCHEMA")
    ...
    if (
        ...
        _fail("FR0010_ATTESTATION_ENVELOPE")
    try:
        ...
        _fail("FR0010_ATTESTATION_STATEMENT")
...
def _expected_attestation_statement(
    *,
    ...
    workflow_identity = (
        f"https://github.com/{REPOSITORY}/{contract['workflow_path']}"
        f"@{expected_ref}"
    )
    return {
        ...
        "predicate": {
            "buildDefinition": {
                ...
                "resolvedDependencies": [
                    {
                        "uri": (
                            f"git+https://github.com/{REPOSITORY}@{expected_ref}"
                        ),
...
def validate_attestation_evidence(
    ...
    *,
    ...
    if (
        ...
        or type(attempt) is not int
        or not 1 <= attempt <= MAX_EPOCH11_RUN_ATTEMPT
    ):
        _fail("FR0010_ATTESTATION_IDENTITY")
    ...
    if canonical_json_bytes(parsed["statement"]) != canonical_json_bytes(
        ...
        _fail("FR0010_ATTESTATION_STATEMENT_MISMATCH")
    if (
        ...
        _fail("FR0010_ATTESTATION_RECEIPT")
    ...
    if (
        ...
        _fail("FR0010_ATTESTATION_RECEIPT_BUNDLE")
    ...
    if (
        ...
        _fail("FR0010_ATTESTATION_VERIFICATION_RESULT")
    ...
    if (
        ...
        _fail("FR0010_ATTESTATION_TRANSPARENCY_LOG")
    ...
    if (
        ...
        _fail("FR0010_ATTESTATION_TRANSPARENCY_LOG_TIME")
    ...
    if not isinstance(certificate, dict) or any(
        ...
        _fail("FR0010_ATTESTATION_CERTIFICATE")
    return {
...
def review_subject_manifest(
    *,
    ...
    if (
        review_id not in {"FR-0010-R01", "FR-0010-R02"}
        or model not in {"claude-fable-5", "claude-opus-5"}
        or HEX40.fullmatch(repair_commit) is None
        or any(HEX64.fullmatch(item) is None for item in (plan_sha256, patch_sha256, gate_sha256))
        or not isinstance(required_findings, list)
        or not required_findings
    ):
        _fail("FR0010_REVIEW_SUBJECT")
    ...
    *,
    ...
    for label, payload in (
        ...
        if (
            ...
            _fail("FR0010_REVIEW_MATERIAL:" + label)
        try:
    ...
    if model not in {"claude-fable-5", "claude-opus-5"}:
        _fail("FR0010_REVIEW_MODEL")
    instruction = (
        "Review only the authenticated FR-0010 repair material. Return one JSON "
        "object with exactly review_id, verdict, required_findings, and "
        "additional_findings. Preserve every required finding ID and order. Use "
        "GO_FOR_FRAMEWORK_QUALIFICATION only when all blocking findings are "
        "resolved by the supplied bytes. Do not claim human review, external "
        "independence, or release authority."
    )
    content = (
        instruction
        + "\n\n-----BEGIN FR-0010 SUBJECT-----\n"
        + canonical_json_bytes(manifest).decode("utf-8")
        + "-----END FR-0010 SUBJECT-----\n"
        + "-----BEGIN FR-0010 PLAN-----\n"
        + plan_payload.decode("utf-8")
        + "-----END FR-0010 PLAN-----\n"
        + "-----BEGIN FR-0010 CORE PATCH-----\n"
        + patch_payload.decode("utf-8")
        + "-----END FR-0010 CORE PATCH-----\n"
    ...
    if len(request) > MAX_REVIEW_REQUEST_BYTES:
        _fail("FR0010_REVIEW_REQUEST_BOUND")
    return request
...
def _bounded_text(value: Any, *, label: str) -> str:
    if (
        ...
        _fail("FR0010_REVIEW_TEXT:" + label)
    return value
...
def _validate_paths(value: Any, *, label: str) -> list[str]:
    if (
        ...
        or any(
            ...
            or REPOSITORY_PATH.fullmatch(path) is None
            for path in value
        )
    ):
        _fail("FR0010_REVIEW_PATHS:" + label)
    return list(value)
...
def _validate_required_finding_contract(value: Any) -> list[dict[str, Any]]:
    if (
        ...
        _fail("FR0010_REVIEW_FINDING_CONTRACT")
...
def _validate_review_finding(
    ...
    *,
    ...
    if not isinstance(value, dict) or set(value) != fields:
        _fail("FR0010_REVIEW_FINDING:" + expected_id)
    finding_id = value["id"]
    if (
    ...
    return {
        ...
        "status": value["status"],
        "summary": _bounded_text(
            value["summary"], label=f"{finding_id}.summary"
        ),
        "affected_paths": _validate_paths(
...
def validate_review_response(
    ...
    if (
        ...
        _fail("FR0010_REVIEW_RESPONSE")
    contract = _validate_required_finding_contract(
        manifest.get("required_findings")
    )
    ...
    if (
        ...
        _fail("FR0010_REVIEW_RESPONSE_FINDINGS")
    normalized_required = [
    ...
    for expected, observed in zip(
        contract, normalized_required, strict=True
    ):
        if observed["affected_paths"] != expected["affected_paths"]:
            _fail("FR0010_REVIEW_RESPONSE_PATHS:" + expected["id"])
    open_ids = [
        item["id"]
    ...
    if (
        response["verdict"] == "GO_FOR_FRAMEWORK_QUALIFICATION"
        and open_ids
    ) or (response["verdict"] == "NO_GO" and not open_ids):
        _fail("FR0010_REVIEW_RESPONSE_VERDICT")
    return {
...
def parse_review_response_bytes(
    ...
    *,
    ...
    if type(raw) is not bytes or not 1 <= len(raw) <= MAX_REVIEW_RESPONSE_BYTES:
        _fail("FR0010_REVIEW_RESPONSE_BOUND")
    try:
        value = json.loads(raw)
    except (UnicodeDecodeError, json.JSONDecodeError):
        _fail("FR0010_REVIEW_RESPONSE_JSON")
