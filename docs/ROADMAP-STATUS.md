...
- Haldir is now registered as an exact-revision NCP consumer and the optional
  ...
  exposes a local safe-boundary request latch, without an in-flight timeout or signal runner. No
  runnable Gate executable/service package for this aggregate authenticates the local inputs,
  opens the session/credentials, supplies
...
- Gate's off-by-default live feature now compiles a capability-marked consuming
  ...
  latching without request-driven cancellation. A direct helper test proves idle wake. The concrete types compile, but no runnable
...
1. **Authority/evidence:** first add a separate, off-by-default development smoke target that
   hard-selects declared-live exact NCP v0.8, requires an already-provisioned disposable state and
   journal plus external ephemeral router credentials, performs explicitly local fixture activation,
   opens one strict session, binds the real aggregate, and immediately shuts it down under an outer
   package-instance lock. Treat that only as concrete bind/cleanup evidence, never as authenticated
   deployment, ongoing control, publication, supervision, or production evidence. Then turn the
