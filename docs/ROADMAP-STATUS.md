...
| 9 — NCP v0.8.0 adapter | Partial | The immutable baseline, modeled adapter, opt-in exact `ncp-core` JSON/frozen-corpus differential path, closed adapter selection, explicit template runtime profile, and always-on pinned-NCP route builders are tested (`CL-NCP-REAL-01`, `CL-TRANSPORT-BOUNDARY-01`). `DeclaredLiveZenoh` requires exact JSON and the compiled live feature before the listed startup-owned backend calls, entropy, locks, or directory access; successful startup privately mints the capability required by the live coordinator typestate, while exact `InProcessReference` and copied-report paths cannot. An exact-selected actor reaches Called with upstream JSON, and valid exact JSON traverses the synthetic live campaign. A no-network activation typestate now derives the exact intent route from the verified/admission-bound lease controller and rejects a differing signed route before authority commits. The public service consumes only that move-only route binding plus one preconstructed publisher, rejects a final-command route outside the actor's exact realm/session before processing, and returns the sole service owner only on safe continuation. No executable/package authenticates ongoing control/state input, selects the profile, opens/owns the session, or drives ingress (`CL-GATE-LIFECYCLE-01`; live-session limits remain under `CL-LIVE-TRANSPORT-01`). |
| 10 — Gate runtime, queues, journal, receipts | Partial | The 13-stage actor, fallible configuration, boot/store-bound startup, declared-live validation, signed receipts, post-sync-revalidated publication seam, canonical linked stages, bounded locked manager/reservations, assurance replay, and fused restart Unknown emission exist. The consuming coordinator reserves three lifecycle records before mutation, sync-orders receipt/Called/terminal transitions, and blocks after recovered called-or-later history pending external clearance. Its live marker descends from a startup-minted move-only capability; only live Called exposes the concrete publisher method. A public no-network kernel now fail-stop primes an initially inactive marked actor with bounded caller-supplied initial state/challenge/signed-lease input; canonical lease-route rejection happens before challenge consumption, durable term commit, revision change, or activation. A public non-cloneable service then consumes only the route-bound result and a preconstructed matched publisher, owns a private fixed capacity-one pool, and accepts one raw, publicly constructible ingress event only after enforcing hard envelope and route-length bounds before capacity, clock, or actor work. It returns itself only after no-output, pre-Called rejection/unavailability, or publisher `Ok` plus confirmed terminal journal. Fatal, cancellation, publisher-error, and terminal-boundary-failure paths return no service/publisher capability. Tests cover activation input bounds/state/clock/canonical-route ordering, route-bound-to-fake-publisher binding, service input/capacity/result/drop/fault ownership, and `Send` surfaces; lower coordinator tests cover result/reopen ambiguity and panic (`CL-GATE-LIFECYCLE-01`). The concrete service/future signatures compile, but no service test opens a live session or invokes the concrete path. Missing gates include authenticated ongoing control/state/lease/revocation ingress, an executable/durably bound service package, session/ingress ownership, a separate bounded async publisher worker/queue, canonical journal-path binding, Prepared abandonment/retention/loss summary, OS append/write/`sync_data` and disk-full fault injection, live-session cancellation/timeout/panic, panic-abort/supervisor behavior, and child-process crash recovery (`CL-DURABLE-01`). |
...
- Haldir is now registered as an exact-revision NCP consumer and the optional
  ...
  capability only from the verified controller; the service consumes that result plus a
  preconstructed matched publisher. No executable/package authenticates their provenance or
  delivery, supplies ongoing controls, or selects this path.
...
- Gate's off-by-default live feature now compiles a capability-marked consuming
  ...
  and a public single-owner service kernel with one private output slot. Tests start from an
  inactive test-minted marked actor and actual journal manager, exercise bounded caller-supplied local
  activation and canonical intent-route binding, then use the shared fake-publisher core;
  Called/result/drop/fault cases still use test publisher seams. No executable, live session,
  authenticated ongoing control path, or concrete publisher invocation exercises the service yet.
...
4. **Operations/security:** make a session/intent-ingress aggregate consume the accepted-lease-
   derived route capability without any caller controller override, then turn it into a package
   that owns authenticated ongoing challenge/lease/revocation/state ingress and one bounded
