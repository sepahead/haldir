...
| 10 — Gate runtime, queues, journal, receipts | Partial | The 13-stage actor, fallible configuration, boot/store-bound startup, signed receipts, in-memory single-slot publication typestate, canonical linked publication-stage primitives, a bounded locked evidence directory manager with ordered opaque recovery snapshots, an assurance-only Gate COSE adapter/fresh offline publication replay, and a staged fused `RunningGate` current-journal binding exist (`CL-CONFIG-01`, `CL-PUBLICATION-STATE-01`, `CL-PUBLICATION-EVIDENCE-PRIMITIVE-01`, `CL-GATE-JOURNAL-REPLAY-01`, `CL-GATE-JOURNAL-BINDING-01`, `CL-DURABLE-STARTUP-DEV-01`, `CL-EVIDENCE-MANAGER-01`); service/package loading, bounded async queues, durable canonical journal-path selection, lifecycle capacity reservation, current-boot Unknown append coordination, and crash-tested runtime recovery do not (`CL-DURABLE-01`). |
