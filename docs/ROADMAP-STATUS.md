...
selection of the exact NCP/internal strict transport path, retained live evidence for the new
development-only bind/shutdown target, certificate-lifecycle/
...
- Haldir is now registered as an exact-revision NCP consumer and the optional
  ...
  development smoke examples now select the path only for disposable local provisioning followed
  by `OpenExisting` bind/immediate shutdown. They do not authenticate the local inputs, protect
  credential custody, supply ongoing controls, or establish retained live evidence.
...
- Gate's off-by-default live feature now compiles a capability-marked consuming
  ...
  method, so it cannot invoke the publisher. No retained live session/subscriber run or authenticated
  ongoing control path exercises the aggregate yet (`CL-LIVE-GATE-DEV-BIND-01`).

## Next completion slice, reviewed from five lenses

1. **Authority/evidence:** run and independently verify a separate clean-source retained campaign
   for the implemented off-by-default pair: the offline provisioner must create the disposable state
   and journal, then the `OpenExisting`-only target must use external ephemeral Gate credentials,
   perform explicitly local fixture activation, open one strict session, bind the real aggregate,
   and immediately shut it down under its outer lock. Treat that only as concrete local bind/cleanup
   evidence, never as authenticated deployment, ongoing control, publication, supervision, or
   production evidence (`CL-LIVE-GATE-DEV-BIND-01`). Then turn the
   startup library into an authenticated secret/package-loading service, add a real external anchor,
   and make the tested durable evidence manager mandatory before authority can become active.
