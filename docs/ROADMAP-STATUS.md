...
1. **Authority/evidence:** preserve the retained development smoke and treat it only as concrete
   local bind/shutdown evidence, never authenticated deployment, ongoing control, publication,
   supervision, or production evidence (`CL-LIVE-GATE-DEV-BIND-01`). Next add strict authenticated
   deployment-package verification, exact referenced-artifact verification, and an atomic global
   per-Gate revision/payload-digest plus boot ratchet. Then add protected credential opening, a real
   external anchor, and mandatory durable evidence-manager selection before authority can become
   active. The full deployment-package/control-plane claims remain unproven until those later gates.
