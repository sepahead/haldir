...
- Haldir is now registered as an exact-revision NCP consumer and the optional
  ...
  sampling. The declaration remains process-local. A public service kernel can consume the
  marked coordinator and a preconstructed matched publisher, but no executable/package selects it.
...
- Gate's off-by-default live feature now compiles a capability-marked consuming
  coordinator-to-concrete-publisher binding plus a public single-owner service kernel with
  one private output slot. Production declared-live startup code with injected in-memory
  backends and the actual journal manager reaches live coordinator construction; activated
  service/Ready/Called/result/drop/fault tests use test-only actor or publisher seams. No
  executable, live session, or concrete publisher invocation exercises the service yet.
...
4. **Operations/security:** turn the single-owner service kernel into a package that owns
   session/intent ingress plus control-plane/lease/state ingress and one bounded publisher
   worker; make that package select and preserve the existing declared-live startup profile
