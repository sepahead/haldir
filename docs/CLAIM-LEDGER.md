...
| Claim | Statement | Status | Evidence |
| --- | --- | --- | --- |
| CL-DURABLE-01 | Anti-rollback high-water and evidence survive a crash via durable authenticated storage. | OUT OF SCOPE | P0 state now has checked exhaustion, canonical decode, and copy-on-write candidates for commit-before-authority ordering, but no durable authenticated store is selected by Gate — see `docs/LIMITATIONS.md` |
