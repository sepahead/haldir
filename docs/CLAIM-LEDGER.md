...
| Claim | Statement | Status | Evidence |
| --- | --- | --- | --- |
| CL-DURABLE-01 | Anti-rollback high-water and evidence survive a crash via durable authenticated storage. | OUT OF SCOPE | Authenticated snapshot/anchor primitives, a Unix atomic-file mechanism, and a commit-before-mutation boot/high-water wrapper exist (`CL-DURABLE-PRIMITIVE-01`), but Gate selects none of them, no external non-rewindable anchor implementation is deployed, and evidence remains in-memory — see `docs/LIMITATIONS.md` |
