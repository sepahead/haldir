...
| CL-DURABLE-PRIMITIVE-01 | The storage primitive authenticates store/Gate/generation/predecessor/payload with HMAC-SHA256, requires explicit provisioning, reconciles only an authenticated exactly-next snapshot with an external compare-and-set anchor, and rejects missing anchors, rewind, fork, generation gaps, wrong keys/bindings, tampering, and pre-existing provisioning. The anti-rollback wrapper changes live high-water state only after its snapshot commit returns. This is an abstract/in-memory-tested mechanism, not a filesystem or Gate-runtime durability claim. | PROVEN | `haldir-durable` RFC 4231/envelope/recovery tests; `haldir-state::durable` failure-order/reopen tests |
...
| Claim | Statement | Status | Evidence |
| --- | --- | --- | --- |
| CL-DURABLE-01 | Anti-rollback high-water and evidence survive a crash via durable authenticated storage. | OUT OF SCOPE | Authenticated snapshot/anchor primitives and a commit-before-mutation anti-rollback wrapper exist (`CL-DURABLE-PRIMITIVE-01`), but Gate selects neither a real atomic filesystem backend nor an external non-rewindable anchor; evidence remains in-memory — see `docs/LIMITATIONS.md` |
