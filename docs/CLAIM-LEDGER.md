...
| Claim | Statement | Status | Evidence |
| --- | --- | --- | --- |
| CL-DURABLE-01 | Anti-rollback high-water and evidence survive a crash via durable authenticated storage. | OUT OF SCOPE | Authenticated snapshot/anchor primitives, a Unix atomic-file mechanism, a commit-before-mutation boot/high-water wrapper, checked Gate injection, and a signed evidence-segment/recovery primitive exist (`CL-DURABLE-PRIMITIVE-01`, `CL-EVIDENCE-SEGMENT-PRIMITIVE-01`), but no runnable service provisions/selects a deployed external non-rewindable anchor or the journal, and no child-process crash campaign exists — see `docs/LIMITATIONS.md` |
